...
class TestQuartoReportGenerator:
    ...
    def sample_model_results(self):
        ...
        return {
            "model_type": "ARIMA",
            "order": (2, 1, 2),
            "model_info": {"aic": 245.67, "bic": 258.89, "n_observations": 100, "status": "fitted"},
            "metrics": {"mae": 2.34, "rmse": 3.45, "mape": 5.67, "r2_score": 0.89},
            "training_data": {
    ...
    def sample_plots_data(self, temp_output_dir):
        ...
        return {"forecast": str(plot1), "residuals": str(plot2)}
    ...
    def test_make_json_serializable(self, temp_output_dir):
        ...
        nested = {"array": np.array([1, 2, 3]), "scalar": np.float64(2.5), "normal": "string"}
        result = generator._make_json_serializable(nested)
        expected = {"array": [1, 2, 3], "scalar": 2.5, "normal": "string"}
    ...
    @pytest.mark.skipif(not HAS_REPORTING, reason="Reporting dependencies not available")
    @patch("subprocess.run")
    def test_generate_model_report_full(
        self, mock_subprocess, temp_output_dir, sample_model_results, sample_plots_data
    ):
...
class TestARIMAForecasterReporting:
    ...
    def test_generate_report_success(self, mock_generate, fitted_arima_model):
        ...
        report_path = fitted_arima_model.generate_report(
            report_title="Test ARIMA Report", format_type="html"
        )
    ...
    def test_generate_report_with_plots(self, fitted_arima_model):
        ...
        with patch(
            "arima_forecaster.reporting.generator.QuartoReportGenerator.generate_model_report"
        ) as mock_generate:
            mock_generate.return_value = Path("/fake/report.html")

            plots_data = {"forecast": "/fake/forecast.png", "residuals": "/fake/residuals.png"}
...
class TestSARIMAForecasterReporting:
    ...
    def test_sarima_generate_report(self, mock_generate, fitted_sarima_model):
        ...
        report_path = fitted_sarima_model.generate_report(
            report_title="Test SARIMA Report", include_seasonal_decomposition=True
        )
...
class TestReportingIntegration:
    ...
    def test_multiple_format_export(self, temp_dir=None):
        ...
        sample_results = {"model_type": "ARIMA", "order": (1, 1, 1), "metrics": {"aic": 245.67}}
        ...
        with patch("subprocess.run") as mock_subprocess:
            ...
            html_path = generator.generate_model_report(
                model_results=sample_results, output_filename="test_html", format_type="html"
            )
            assert html_path.suffix == ".html"

            # Test PDF (simulato)
            pdf_path = generator.generate_model_report(
                model_results=sample_results, output_filename="test_pdf", format_type="pdf"
            )
