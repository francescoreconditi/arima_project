...
class TestForecasting:
    ...
    def test_generate_forecast_success(
        self, client: TestClient, trained_model_id, sample_forecast_request
    ):
        """Test generazione forecast con successo."""
        response = client.post(f"/models/{trained_model_id}/forecast", json=sample_forecast_request)
    ...
    def test_generate_forecast_without_confidence_intervals(
        self, client: TestClient, trained_model_id
    ):
        """Test forecast senza intervalli confidenza."""
        forecast_request = {"steps": 3, "return_confidence_intervals": False}

        response = client.post(f"/models/{trained_model_id}/forecast", json=forecast_request)
    ...
    def test_generate_forecast_different_confidence_levels(
        self, client: TestClient, trained_model_id
    ):
    ...
    def test_forecast_nonexistent_model(self, client: TestClient):
        ...
        response = client.post(f"/models/{fake_model_id}/forecast", json=forecast_request)
    ...
    def test_forecast_invalid_steps(self, client: TestClient, trained_model_id):
        ...
        for invalid_request in invalid_requests:
            response = client.post(f"/models/{trained_model_id}/forecast", json=invalid_request)
    ...
    def test_forecast_numeric_types(self, client: TestClient, trained_model_id):
        ...
        if data["confidence_intervals"]:
            for lower, upper in zip(
                data["confidence_intervals"]["lower"], data["confidence_intervals"]["upper"]
            ):
...
class TestForecastingPerformance:
    ...
    def test_multiple_concurrent_forecasts(self, client: TestClient, trained_model_id):
        ...
        def make_forecast():
            return client.post(f"/models/{trained_model_id}/forecast", json={"steps": 5})
