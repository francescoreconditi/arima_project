...
class TestSARIMAXForecaster:
    ...
    def sample_data(self):
        ...
        exog = pd.DataFrame(
            {
                "temp": 20
                + 5 * np.sin(2 * np.pi * np.arange(100) / 365)
    ...
    def simple_forecaster(self):
        ...
        return SARIMAXForecaster(
            order=(1, 1, 1), seasonal_order=(1, 1, 1, 7), exog_names=["temp", "marketing"]
        )

    def test_init(self):
        """Test inizializzazione modello SARIMAX."""
        forecaster = SARIMAXForecaster(
            order=(2, 1, 1), seasonal_order=(1, 0, 1, 12), exog_names=["var1", "var2"], trend="c"
        )
    ...
    def test_forecast_with_exog(self, simple_forecaster, sample_data):
        ...
        exog_future = pd.DataFrame({"temp": [22, 23, 21], "marketing": [1200, 1300, 1400]})

        forecast = simple_forecaster.forecast(steps=3, exog_future=exog_future)
    ...
    def test_forecast_with_confidence_intervals(self, simple_forecaster, sample_data):
        ...
        forecast, conf_int = simple_forecaster.forecast(
            steps=3, exog_future=exog_future, confidence_intervals=True, return_conf_int=True
        )
    ...
    def test_predict_with_exog(self, simple_forecaster, sample_data):
        ...
        predictions = simple_forecaster.predict(start=0, end=9, exog=exog_subset)
    ...
    def test_save_and_load(self, simple_forecaster, sample_data):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            pd.testing.assert_frame_equal(
                loaded_forecaster.training_exog, simple_forecaster.training_exog
            )
    ...
    def test_save_not_fitted(self, simple_forecaster):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with pytest.raises(
                ModelTrainingError, match="Nessun modello SARIMAX addestrato da salvare"
            ):
                simple_forecaster.save(save_path)
    ...
    def test_generate_report_functionality(self, simple_forecaster, sample_data):
        ...
        with patch(
            "arima_forecaster.core.sarimax_model.QuartoReportGenerator", side_effect=ImportError
        ):
...
class TestSARIMAXIntegration:
    ...
    def test_complete_workflow(self):
        ...
        # Variabili esogene
        temperature = (
            20 + 10 * np.sin(2 * np.pi * np.arange(200) / 365) + np.random.normal(0, 2, 200)
        )
        marketing = 1000 + np.random.normal(0, 200, 200)
        exog = pd.DataFrame({"temperature": temperature, "marketing": marketing}, index=dates)
        ...
        model = SARIMAXForecaster(
            order=(1, 1, 1), seasonal_order=(1, 0, 1, 7), exog_names=["temperature", "marketing"]
        )
