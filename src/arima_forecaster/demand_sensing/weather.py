...
class WeatherImpact(BaseModel):
    ...
    temp_impact_per_degree: float = Field(
        0.02, description="Impatto % per grado di differenza dalla media"
    )
...
class WeatherIntegration:
    ...
    def fetch_forecast(
        self, days_ahead: int = 7, use_demo_data: bool = False
    ...
    def _generate_demo_forecast(self, days: int) -> List[WeatherCondition]:
        ...
        hist = self.historical_avg.get(self.location, self.historical_avg["default"])
    ...
    def calculate_weather_impact(
        ...
        sensitivity = self.impact_config.product_sensitivity.get(
            product_category, self.impact_config.product_sensitivity["default"]
        )

        # Media storica
        hist_temp = self.historical_avg.get(self.location, self.historical_avg["default"])["temp"]

        for condition in conditions:
            # Calcola impatto temperatura
            temp_diff = condition.temperature - hist_temp
            temp_impact = (
                temp_diff * self.impact_config.temp_impact_per_degree * sensitivity
            ) / 100
    ...
    def get_extreme_weather_alerts(self, conditions: List[WeatherCondition]) -> List[str]:
        """
