...
class Event(BaseModel):
    ...
    type: EventType
    duration_days: int = 1
    impact_radius_days: int = Field(
        7,
        description="Giorni prima/dopo con impatto"
    )
    expected_impact: float = Field(
        0.1,
        description="Impatto atteso su domanda (-1 a 1)"
    )
...
class CalendarEvents:
    ...
    def _init_holidays(self):
        ...
        self.holidays = {
            'IT': [
                Event(name="Capodanno", date=datetime(2025, 1, 1), 
                     type=EventType.HOLIDAY, impact_radius_days=3, expected_impact=0.15),
                Event(name="Epifania", date=datetime(2025, 1, 6), 
                     type=EventType.RELIGIOUS, impact_radius_days=2, expected_impact=0.10),
                Event(name="Pasqua", date=datetime(2025, 4, 20), 
                     type=EventType.RELIGIOUS, impact_radius_days=5, expected_impact=0.20),
                Event(name="Pasquetta", date=datetime(2025, 4, 21), 
                     type=EventType.HOLIDAY, impact_radius_days=2, expected_impact=0.12),
                Event(name="Festa della Liberazione", date=datetime(2025, 4, 25), 
                     type=EventType.HOLIDAY, impact_radius_days=2, expected_impact=0.08),
                Event(name="Festa del Lavoro", date=datetime(2025, 5, 1), 
                     type=EventType.HOLIDAY, impact_radius_days=2, expected_impact=0.08),
                Event(name="Festa della Repubblica", date=datetime(2025, 6, 2), 
                     type=EventType.HOLIDAY, impact_radius_days=2, expected_impact=0.08),
                Event(name="Ferragosto", date=datetime(2025, 8, 15), 
                     type=EventType.HOLIDAY, impact_radius_days=7, expected_impact=0.25),
                Event(name="Ognissanti", date=datetime(2025, 11, 1), 
                     type=EventType.RELIGIOUS, impact_radius_days=2, expected_impact=0.08),
                Event(name="Immacolata", date=datetime(2025, 12, 8), 
                     type=EventType.RELIGIOUS, impact_radius_days=3, expected_impact=0.15),
                Event(name="Natale", date=datetime(2024, 12, 25), 
                     type=EventType.RELIGIOUS, impact_radius_days=10, expected_impact=0.35),
                Event(name="Santo Stefano", date=datetime(2024, 12, 26), 
                     type=EventType.HOLIDAY, impact_radius_days=2, expected_impact=0.15),
                Event(name="San Silvestro", date=datetime(2024, 12, 31), 
                     type=EventType.HOLIDAY, impact_radius_days=3, expected_impact=0.20),
                
                # Eventi commerciali
                Event(name="Black Friday", date=datetime(2024, 11, 29), 
                     type=EventType.BLACKFRIDAY, impact_radius_days=5, expected_impact=0.40),
                Event(name="Cyber Monday", date=datetime(2024, 12, 2), 
                     type=EventType.BLACKFRIDAY, impact_radius_days=2, expected_impact=0.30),
                Event(name="Saldi Invernali", date=datetime(2025, 1, 5), 
                     type=EventType.SEASONAL, duration_days=30, impact_radius_days=5, expected_impact=0.25),
                Event(name="Saldi Estivi", date=datetime(2025, 7, 1), 
                     type=EventType.SEASONAL, duration_days=30, impact_radius_days=5, expected_impact=0.25),
                Event(name="San Valentino", date=datetime(2025, 2, 14), 
                     type=EventType.HOLIDAY, impact_radius_days=5, expected_impact=0.15),
                Event(name="Festa della Mamma", date=datetime(2025, 5, 11), 
                     type=EventType.HOLIDAY, impact_radius_days=5, expected_impact=0.12),
                Event(name="Festa del Papà", date=datetime(2025, 3, 19), 
                     type=EventType.HOLIDAY, impact_radius_days=3, expected_impact=0.10),
            ],
            'US': [
                Event(name="New Year", date=datetime(2025, 1, 1), 
                     type=EventType.HOLIDAY, expected_impact=0.15),
                Event(name="MLK Day", date=datetime(2025, 1, 20), 
                     type=EventType.HOLIDAY, expected_impact=0.05),
                Event(name="Presidents Day", date=datetime(2025, 2, 17), 
                     type=EventType.HOLIDAY, expected_impact=0.08),
                Event(name="Easter", date=datetime(2025, 4, 20), 
                     type=EventType.RELIGIOUS, expected_impact=0.15),
                Event(name="Memorial Day", date=datetime(2025, 5, 26), 
                     type=EventType.HOLIDAY, expected_impact=0.12),
                Event(name="Independence Day", date=datetime(2025, 7, 4), 
                     type=EventType.HOLIDAY, expected_impact=0.20),
                Event(name="Labor Day", date=datetime(2025, 9, 1), 
                     type=EventType.HOLIDAY, expected_impact=0.10),
                Event(name="Thanksgiving", date=datetime(2024, 11, 28), 
                     type=EventType.HOLIDAY, impact_radius_days=5, expected_impact=0.30),
                Event(name="Black Friday", date=datetime(2024, 11, 29), 
                     type=EventType.BLACKFRIDAY, impact_radius_days=5, expected_impact=0.45),
                Event(name="Christmas", date=datetime(2024, 12, 25), 
                     type=EventType.RELIGIOUS, impact_radius_days=10, expected_impact=0.35),
            ],
            'default': [
                Event(name="New Year", date=datetime(2025, 1, 1), 
                ...
                Event(name="Easter", date=datetime(2025, 4, 20), 
                     type=EventType.RELIGIOUS, expected_impact=0.15),
                Event(name="Christmas", date=datetime(2024, 12, 25), 
                     type=EventType.RELIGIOUS, expected_impact=0.30),
            ]
        }
    ...
    def get_events(
        ...
        for event in all_events:
            ...
            event_end = event.date + timedelta(
                days=event.duration_days + event.impact_radius_days
            )
    ...
    def calculate_event_impact(
        ...
        category_sens = self.impact_config.category_sensitivity.get(
            ...
            self.impact_config.category_sensitivity['default']
        )
            ...
            for event in events:
                ...
                if abs(days_to_event) <= event.impact_radius_days:
                    ...
                    pattern = self.impact_config.impact_pattern.get(
                        event.type.value,
                        self.impact_config.impact_pattern['default']
                    )
                    ...
                    type_mult = self.impact_config.event_multipliers.get(
                        event.type.value, 0.1
                    )
            ...
            if impacting_events:
                ...
                factor = ExternalFactor(
                    ...
                    metadata={
                        ...
                        'dominant_event': max(impacting_events, 
                                             key=lambda x: abs(x['impact']))['name'],
