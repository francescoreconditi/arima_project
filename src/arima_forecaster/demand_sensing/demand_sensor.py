...
class SensingConfig(BaseModel):
    ...
    strategy: AdjustmentStrategy = Field(
        AdjustmentStrategy.WEIGHTED, description="Strategia di aggiustamento"
    )
    max_adjustment: float = Field(0.3, description="Massimo aggiustamento permesso (30%)")
    min_confidence: float = Field(0.3, description="Confidenza minima per applicare aggiustamento")
    ...
    historical_weight: float = Field(
        0.7, description="Peso dato alle previsioni storiche vs fattori esterni"
    )
    learning_rate: float = Field(
        0.1, description="Tasso di apprendimento per aggiustamenti adattivi"
    )
        ...
        df = pd.DataFrame(
            {
                ...
                "adjustment_pct": (
                    (self.adjusted_forecast - self.original_forecast) / self.original_forecast * 100
                ),
            }
        )
        return df
...
class DemandSensor:
    ...
    def calculate_impact(self, factor: ExternalFactor, base_forecast: float) -> FactorImpact:
        """
        ...
        adjustment_percentage = (
            (adjustment_amount / base_forecast * 100) if base_forecast != 0 else 0
        )
    ...
    def combine_impacts(self, impacts: List[FactorImpact], base_forecast: float) -> float:
        """
        ...
        if self.config.strategy == AdjustmentStrategy.WEIGHTED:
            ...
            weighted_adjustment = (
                sum(i.adjustment_amount * i.factor.confidence for i in impacts) / total_confidence
            )
    ...
    def _fetch_external_factors(self, start_date: datetime, horizon: int) -> None:
        """
    ...
    def _generate_recommendations(self, impacts: List[FactorImpact]) -> List[str]:
        """
    ...
    def learn_from_actuals(
        self, predicted: pd.Series, actuals: pd.Series, factors_used: List[ExternalFactor]
    ) -> None:
        """
        ...
        for factor in factors_used:
            ...
            logger.info(f"Aggiornato peso per {key}: {current_weight:.3f} -> {new_weight:.3f}")
    ...
    def get_factor_importance(self) -> pd.DataFrame:
        ...
        df["importance_score"] = df["avg_impact"] * df["avg_confidence"] * np.log1p(df["count"])
