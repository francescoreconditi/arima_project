...
class SentimentImpact(BaseModel):
    ...
    # Fattore viralità
    virality_threshold: int = Field(10000, description="Soglia interazioni per considerare virale")
    virality_multiplier: float = Field(2.0, description="Moltiplicatore per post virali")

    # Influencer impact
    influencer_multiplier: float = Field(1.5, description="Moltiplicatore per post influencer")

    # Decay temporale
    impact_decay_days: int = Field(7, description="Giorni per dimezzare impatto")
...
class SocialSentimentAnalyzer:
    ...
    def fetch_social_posts(
        self, platforms: List[str] = None, days_back: int = 7, use_demo_data: bool = True
    ) -> List[SocialPost]:
        """
    ...
    def _generate_demo_posts(self, platforms: List[str], days_back: int) -> List[SocialPost]:
        """
    ...
    def analyze_sentiment(self, text: str) -> float:
        ...
        # Calcola score
        total_signals = positive_count + negative_count + positive_emoji + negative_emoji
    ...
    def calculate_social_impact(
        self, posts: List[SocialPost], forecast_horizon: int = 7
    ) -> List[ExternalFactor]:
        """
        ...
        for day in range(forecast_horizon):
            ...
            recent_dates = [forecast_date.date() - timedelta(days=i) for i in range(1, 8)]

            recent_sentiments = [avg_sentiments.get(d, 0) for d in recent_dates]
            ...
            recent_viral = sum(
                ...
                and (p.likes + p.shares) > self.impact_config.virality_threshold
            )
            ...
            n_recent_posts = sum(1 for p in posts if p.timestamp.date() in recent_dates)
            ...
            factor = ExternalFactor(
                ...
                metadata={
                    ...
                    "avg_engagement": np.mean([p.likes + p.shares for p in posts[-50:]])
                    if posts
                    else 0,
                },
            )
    ...
    def get_trending_hashtags(
        self, posts: List[SocialPost], top_n: int = 10
    ) -> List[Tuple[str, int]]:
        """
    ...
    def identify_influencers(
        self, posts: List[SocialPost], min_engagement: int = 5000
    ) -> List[str]:
        """
        ...
        influencers = [
            author
            for author, total_eng in author_engagement.items()
