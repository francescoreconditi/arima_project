...
class TrendImpact(BaseModel):
    ...
    impact_multiplier: float = Field(
        0.005,
        description="Impatto per punto percentuale di interesse"
    )
    spike_multiplier: float = Field(
        2.0,
        description="Moltiplicatore aggiuntivo per spike"
    )
    
    # Lag temporale (giorni tra ricerca e acquisto)
    purchase_lag_days: int = Field(
        3,
        description="Giorni medi tra ricerca e acquisto"
    )
...
class GoogleTrendsIntegration:
    ...
    def calculate_trend_impact(
        ...
        category_weight = self.impact_config.category_weights.get(
            product_category,
            self.impact_config.category_weights['default']
        )
        ...
        for day in range(forecast_horizon):
            ...
            avg_interest = recent_trend.mean() if isinstance(recent_trend, pd.Series) else recent_trend
    ...
    def get_related_queries(self, use_demo: bool = False) -> Dict[str, List[str]]:
        ...
        if use_demo or not PYTRENDS_AVAILABLE:
            ...
            demo_related = {
                ...
                'giacca invernale': ['piumino', 'cappotto donna', 'giubbotto uomo', 'saldi giacche'],
                'pizza': ['pizza domicilio', 'pizzeria vicino', 'pizza napoletana', 'pizza margherita'],
                'concerto': ['biglietti concerto', 'eventi milano', 'concerti 2024', 'ticketone'],
                'default': ['prodotto simile', 'alternativa', 'offerta', 'recensioni']
            }
            
            for keyword in self.keywords:
                related[keyword] = demo_related.get(
                    keyword,
                    demo_related['default']
                )
        else:
            try:
                for keyword in self.keywords:
