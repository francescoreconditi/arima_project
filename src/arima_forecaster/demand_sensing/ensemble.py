...
class EnsembleDemandSensor:
    ...
    def __init__(
        ...
        logger.info(f"EnsembleDemandSensor inizializzato per {product_category} in {location}")
    ...
    def _init_modules(self):
        ...
        if self.config.enable_weather:
            self.weather = WeatherIntegration(location=self.location, impact_config=WeatherImpact())
        else:
            self.weather = None
        ...
        if self.config.enable_trends:
            ...
            self.trends = GoogleTrendsIntegration(
                keywords=keywords, geo=self.location.split(",")[1] if "," in self.location else "IT"
            )
        else:
            self.trends = None
        ...
        if self.config.enable_social:
            ...
            self.social = SocialSentimentAnalyzer(
                brand_keywords=brand_keywords, product_keywords=self._get_category_keywords()
            )
        else:
            self.social = None
        ...
        if self.config.enable_economic:
            ...
            self.economic = EconomicIndicators(country=country, sector=self._get_economic_sector())
        else:
            self.economic = None
        ...
        if self.config.enable_calendar:
            country = self.location.split(",")[1] if "," in self.location else "IT"
            self.calendar = CalendarEvents(country=country, product_category=self.product_category)
        else:
            self.calendar = None
    ...
    def collect_all_factors(
        self, forecast_horizon: int = 30, use_demo_data: bool = True
        ...
        if self.weather:
            try:
                conditions = self.weather.fetch_forecast(
                    days_ahead=forecast_horizon, use_demo_data=use_demo_data
                )
                weather_factors = self.weather.calculate_weather_impact(
                    conditions, product_category=self.product_category
                )
        ...
        if self.trends:
            try:
                trend_data = self.trends.fetch_trends(
                    timeframe="today 1-m", use_demo_data=use_demo_data
                )
        ...
        if self.social:
            try:
                posts = self.social.fetch_social_posts(days_back=7, use_demo_data=use_demo_data)
                social_factors = self.social.calculate_social_impact(
                    posts, forecast_horizon=forecast_horizon
                )
        ...
        if self.economic:
            try:
                indicators = self.economic.fetch_indicators(use_demo_data=use_demo_data)
                economic_factors = self.economic.calculate_economic_impact(
                    indicators, forecast_horizon=forecast_horizon
                )
        ...
        if self.calendar:
            try:
                ...
                calendar_factors = self.calendar.calculate_event_impact(
                    events, forecast_horizon=forecast_horizon
                )
    ...
    def combine_factors(
        self, factors_by_source: Dict[str, List[ExternalFactor]]
    ) -> List[ExternalFactor]:
        """
    ...
    def sense(
        ...
        all_factors = self.collect_all_factors(
            forecast_horizon=forecast_horizon, use_demo_data=use_demo_data
        )
        ...
        ensemble_recommendations = self._generate_ensemble_recommendations(all_factors, result)
    ...
    def _generate_ensemble_recommendations(
        self, all_factors: Dict[str, List[ExternalFactor]], result: SensingResult
    ) -> List[str]:
        """
    ...
    def learn_from_actuals(self, predicted: pd.Series, actuals: pd.Series) -> None:
        """
    ...
    def plot_sensing_analysis(self, result: SensingResult, save_path: Optional[str] = None) -> None:
        """
        ...
        ax1 = axes[0, 0]
        ax1.plot(
            result.original_forecast.index,
            result.original_forecast.values,
            label="Original",
            linewidth=2,
        )
        ax1.plot(
            result.adjusted_forecast.index,
        ...
        ax2 = axes[0, 1]
        adjustment_pct = (
            (result.adjusted_forecast - result.original_forecast) / result.original_forecast * 100
        )
        ax2.bar(
            range(len(adjustment_pct)),
            adjustment_pct.values,
