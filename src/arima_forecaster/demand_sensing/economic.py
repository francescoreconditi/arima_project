...
class EconomicImpact(BaseModel):
    ...
    sector_sensitivity: Dict[str, Dict[str, float]] = Field(
        default_factory=lambda: {
            'luxury': {
                'gdp': 1.5, 'consumer_confidence': 2.0,
                'stock_index': 1.8, 'unemployment': -1.5
            },
            'essential': {
                'gdp': 0.3, 'inflation': -0.2,
                'unemployment': -0.3, 'consumer_confidence': 0.5
            },
            'technology': {
                'gdp': 1.0, 'interest_rate': -0.5,
                'stock_index': 1.2, 'consumer_confidence': 1.3
            },
            'retail': {
                'retail_sales': 1.5, 'consumer_confidence': 1.0,
                'unemployment': -0.8, 'inflation': -0.6
            },
            'automotive': {
                'gdp': 1.2, 'interest_rate': -1.0,
                'oil_price': -0.5, 'consumer_confidence': 1.5
            },
            'default': {
                'gdp': 0.8, 'inflation': -0.4,
                'unemployment': -0.5, 'consumer_confidence': 0.8
            }
        }
    )
...
class EconomicIndicators:
    ...
    def _generate_demo_indicators(
        ...
        baseline = self.baseline_values.get(
            self.country,
            self.baseline_values['default']
        )
    ...
    def calculate_economic_impact(
        ...
        sector_sens = self.impact_config.sector_sensitivity.get(
            self.sector,
            self.impact_config.sector_sensitivity['default']
        )
        ...
        for day in range(forecast_horizon):
            ...
            for data in indicators:
                ...
                # Elasticità base
                base_elasticity = self.impact_config.indicator_elasticity.get(
                    indicator_name, 0.5
                )
            ...
            factor = ExternalFactor(
                ...
                metadata={
                    ...
                    'dominant_factor': max(impact_details, 
                                          key=lambda x: abs(x['impact']))['indicator']
                }
            )
