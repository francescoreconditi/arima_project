...
class IntermittentEvaluator:
    ...
    def __init__(self, holding_cost: float = 1.0, stockout_cost: float = 10.0):
        """
    ...
    def evaluate(
        ...
        if np.any(demand_mask):
            ...
            mape_demand = 100 * np.mean(np.abs(errors[demand_mask]) / y_true[demand_mask])
        else:
            mae_demand = 0
            mape_demand = 0

        # Simulazione inventory per service level
        inventory_metrics = self._simulate_inventory(y_true, y_pred, initial_stock)

        # Calcola costi se specificati
        if self.holding_cost > 0 or self.stockout_cost > 0:
            costs = self._calculate_costs(
                inventory_metrics["stock_levels"], inventory_metrics["stockouts"]
            )
    ...
    def _simulate_inventory(
        self, demand: np.ndarray, forecast: np.ndarray, initial_stock: float
    ) -> Dict:
        """
    ...
    def _calculate_costs(
        self, stock_levels: List[float], stockouts: List[float]
    ) -> Dict[str, float]:
        """
        ...
        return {"holding": holding_cost, "stockout": stockout_cost, "total": total_cost}

    def compare_methods(self, actual: np.ndarray, forecasts: Dict[str, np.ndarray]) -> pd.DataFrame:
        """
    ...
    def plot_evaluation(
        self, actual: np.ndarray, forecast: np.ndarray, method_name: str = "Forecast"
    ) -> None:
        """
        ...
        try:
            ...
            ax.text(0.1, 0.5, text, fontsize=10, verticalalignment="center", fontfamily="monospace")
    ...
    def __repr__(self) -> str:
        return (
            f"IntermittentEvaluator(holding_cost={self.holding_cost}, "
