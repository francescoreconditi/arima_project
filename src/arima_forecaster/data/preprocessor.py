...
class TimeSeriesPreprocessor:
    ...
    def handle_missing_values(self, series: pd.Series, method: str = "interpolate") -> pd.Series:
        """
        ...
        self.logger.info(
            f"Valori mancanti gestiti: {series.isnull().sum()} -> {result.isnull().sum()}"
        )

        return result

    def remove_outliers(
        self, series: pd.Series, method: str = "iqr", threshold: float = 3.0
    ) -> pd.Series:
        """
    ...
    def make_stationary(
        self, series: pd.Series, method: str = "difference", max_diff: int = 2
    ) -> Tuple[pd.Series, int]:
        """
        ...
        for i in range(max_diff + 1):
            ...
            if method == "difference":
                ...
                if (current_series <= 0).any():
                    self.logger.warning(
                        "Impossibile applicare trasformazione logaritmica a valori non positivi, uso differenza regolare"
                    )
                    current_series = current_series.diff().dropna()
                else:
    ...
    def preprocess_pipeline(
        ...
        result = series.copy()
        metadata = {"original_length": len(series), "original_missing": series.isnull().sum()}
        ...
        self.logger.info(
            f"Preprocessing completato: {metadata['original_length']} -> {metadata['final_length']} osservazioni"
        )

        return result, metadata

    def validate_exog_data(
        self, series: pd.Series, exog: pd.DataFrame, exog_names: Optional[List[str]] = None
        ...
        validation_report = {"valid": True, "warnings": [], "errors": [], "statistics": {}}
        ...
        try:
            ...
            if not isinstance(exog, pd.DataFrame):
                ...
                validation_report["errors"].append(
                    "Le variabili esogene devono essere un pandas DataFrame"
                )
            ...
            if exog_names is not None:
                ...
                if missing_cols:
                    validation_report["valid"] = False
                    validation_report["errors"].append(
                        f"Variabili esogene mancanti: {missing_cols}"
                    )

                if extra_cols:
                    validation_report["warnings"].append(
                        f"Variabili esogene extra non richieste: {extra_cols}"
                    )

            # Controllo indici temporali
            if not series.index.equals(exog.index):
                validation_report["warnings"].append(
                    "Indici di serie target e variabili esogene non corrispondono"
                )
            ...
            if numeric_cols:
                ...
                for col in numeric_cols:
                    ...
                    stats_summary[col] = {
                        ...
                        "zero_variance": col_stats["std"] < 1e-10
                        if pd.notna(col_stats["std"])
                        else True,
                    }
            ...
            validation_report["statistics"]["summary"] = {
                ...
                "missing_percentage_overall": round(
                    exog.isnull().sum().sum() / (len(exog) * len(exog.columns)) * 100, 2
                ),
            }
    ...
    def preprocess_exog_data(
        ...
        if handle_missing:
            ...
            metadata["processing_steps"].append(
                f"Valori mancanti gestiti: {missing_before} -> {missing_after}"
            )
        ...
        if remove_outliers_flag:
            ...
            if outliers_removed_total > 0:
                metadata["processing_steps"].append(
                    f"Outliers gestiti (capping): {outliers_removed_total}"
                )
        ...
        self.logger.info(
            f"Preprocessing esogeni completato: shape {metadata['original_shape']} -> {metadata['final_shape']}"
        )
    ...
    def preprocess_pipeline_with_exog(
        ...
        if exog is not None:
            ...
            if validate_exog:
                ...
                if not is_valid:
                    critical_errors = [
                        err for err in validation_report["errors"] if "critica" in err.lower()
                    ]
                    if critical_errors:
                        raise DataProcessingError(
                            f"Errori critici nelle variabili esogene: {critical_errors}"
                        )
        ...
        combined_metadata = {
            ...
            "alignment": {
                ...
                "final_alignment_check": (
                    len(processed_series) == len(processed_exog)
