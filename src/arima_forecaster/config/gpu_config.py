...
def detect_gpu_capability() -> GPUCapability:
    ...
    if capability.has_cuda:
        ...
        logger.info(
            f"   - Memoria GPU: {capability.gpu_memory:.1f}GB"
            if capability.gpu_memory
            else "   - Memoria GPU: N/A"
        )
        logger.info(
            f"   - CUDA Version: {capability.cuda_version}"
            if capability.cuda_version
            else "   - CUDA Version: N/A"
        )
    else:
    ...
    if requested_backend.lower() == "auto":
        if capability.has_cuda:
            ...
            if capability.gpu_memory and capability.gpu_memory < 2.0:
                logger.warning(
                    f"GPU memory insufficiente ({capability.gpu_memory:.1f}GB < 2GB) - usando CPU"
                )
                return GPUBackend.CPU
...
class GPUConfig:
    ...
    def _optimize_for_hardware(self):
        ...
        if self.capability.compute_capability and self.capability.compute_capability[0] < 7:
...
def get_gpu_config(backend: str = "auto", **kwargs) -> GPUConfig:
    ...
    config = GPUConfig(backend=optimal_backend, **kwargs)

    logger.info(
        f"GPU Config creata: backend={config.backend.value}, "
...
def set_gpu_memory_limit(limit_gb: float) -> bool:
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            logger.info(
                f"Limite memoria GPU impostato: {limit_gb}GB (fraction: {memory_fraction:.2f})"
            )
            return True
    except Exception as e:
