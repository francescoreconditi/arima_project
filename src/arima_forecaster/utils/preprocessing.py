...
class ExogenousPreprocessor:
    ...
    def fit(self, exog_data: pd.DataFrame) -> "ExogenousPreprocessor":
        ...
        """
        logger.info(
            f"Fitting preprocessore avanzato su {len(exog_data)} osservazioni, {len(exog_data.columns)} variabili"
        )
        ...
        for col in processed_data.columns:
            ...
            if scaler is not None:
                ...
                clean_series = (
                    self._handle_outliers(series, col) if self.handle_outliers else series
                )
                ...
                # Log statistiche
                logger.debug(
                    f"Variable {col}: mean={clean_series.mean():.3f}, std={clean_series.std():.3f}, range=[{clean_series.min():.3f}, {clean_series.max():.3f}]"
                )
    ...
    def transform(self, exog_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in exog_data.columns:
            if col not in self.scalers and col not in self.outlier_bounds:
                logger.warning(
                    f"Variabile {col} non vista durante il fit - uso trasformazione identità"
                )
                continue
    ...
    def suggest_improvements(self, target_series: Optional[pd.Series] = None) -> List[str]:
        ...
        if self.multicollinear_features:
            suggestions.append(
                f"Consider removing multicollinear features: {self.multicollinear_features}"
            )

        # Analisi stazionarietà
        non_stationary = [
            col
            for col, result in self.stationarity_results.items()
        ...
        # Analisi outlier
        high_outlier_vars = [
            col for col in self.outlier_bounds.keys() if len(self.transformation_log) > 0
        ]  # Placeholder logic
        if high_outlier_vars:
            suggestions.append(
                "Consider more robust outlier handling for variables with many outliers"
            )
        ...
        if target_series is not None and hasattr(self, "_last_processed_data"):
            try:
                correlations = (
                    self._last_processed_data.corrwith(target_series)
                    .abs()
                ...
                if low_corr_features:
                    suggestions.append(
                        f"Consider removing low-correlation features: {low_corr_features[:5]}"
                    )
...
def validate_exog_data(exog_data: pd.DataFrame, series_length: int) -> Tuple[bool, str]:
    ...
    try:
        ...
        if len(exog_data) != series_length:
            return (
                False,
                f"Lunghezza variabili esogene ({len(exog_data)}) != lunghezza serie ({series_length})",
            )
...
def suggest_preprocessing_method(exog_data: pd.DataFrame) -> str:
    ...
    try:
        ...
        max_scale_diff = (
            max(scale_differences) - min(scale_differences) if len(scale_differences) > 1 else 0
        )
...
def analyze_feature_relationships(
    exog_data: pd.DataFrame, target_series: pd.Series
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if low_corr:
            results["recommendations"].append(
                f"Low correlation features (consider removing): {low_corr}"
            )
...
def detect_feature_interactions(
    exog_data: pd.DataFrame, max_interactions: int = 10
