...
class ExogDiagnostics:
    ...
    def full_diagnostic_suite(
        self, target_series: pd.Series, exog_data: pd.DataFrame, fitted_model: Optional[Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            results["correlation_analysis"] = self.advanced_correlation_analysis(
                target_series, exog_data
            )

            # Feature importance
            results["feature_importance"] = self.calculate_feature_importance(
                target_series, exog_data
            )
    ...
    def test_stationarity_comprehensive(
        self, target_series: pd.Series, exog_data: pd.DataFrame
    ...
    def _test_single_series_stationarity(self, series: pd.Series, name: str) -> Dict[str, Any]:
        ...
        results = {"series_name": name, "observations": len(series_clean), "tests": {}}
    ...
    def _test_granger_causality_single(
        self, target: pd.Series, exog_var: pd.Series, var_name: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, col in enumerate(exog_aligned.columns):
                results[col] = {
                    ...
                    "f_significant": (f_pvalues[i] < self.significance_level)
                    if i < len(f_pvalues)
                    else False,
                }
            ...
            return {
                ...
                "significant_features": [
                    col for col, data in results.items() if data["f_significant"]
                ],
            }
        ...
        try:
            ...
            if len(exog_data.columns) > 0:
                try:
                    ...
                    residual_index = (
                        residuals.index if hasattr(residuals, "index") else range(len(residuals))
                    )
                    ...
                    if len(common_idx) > 10:
                        ...
                        bp_stat, bp_p, bp_f, bp_f_p = het_breuschpagan(
                            residuals_aligned, exog_aligned.values
                        )
    ...
    def generate_recommendations(self, diagnostic_results: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if "stationarity" in diagnostic_results:
                ...
                if non_stationary:
                    recommendations.append(
                        f"Consider differencing non-stationary variables: {non_stationary}"
                    )
            ...
            if "causality" in diagnostic_results:
                ...
                if non_causal:
                    recommendations.append(
                        f"Consider removing non-causal variables: {non_causal[:3]}"
                    )
                if weak_causal:
            ...
            if "correlation_analysis" in diagnostic_results:
                ...
                if weak_corr:
                    recommendations.append(
                        f"Low correlation features may add noise: {weak_corr[:3]}"
                    )
            ...
            if "residual_analysis" in diagnostic_results:
                ...
                if residual_tests.get("normality", {}).get("normal") == False:
                    recommendations.append(
                        "Residuals are not normal - consider data transformation"
                    )
                ...
                if dw_interp and "autocorrelation" in dw_interp:
                    recommendations.append(
                        "Autocorrelated residuals - consider additional lags or ARMA terms"
                    )
    ...
    def create_diagnostic_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "causality" in results:
                causal_vars = [
                    var for var, res in results["causality"].items() if res.get("is_causal", False)
                ]
                if causal_vars:
                    summary["key_findings"].append(
                        f"Causal variables identified: {len(causal_vars)}"
                    )
            ...
            if "feature_importance" in results:
                ...
                if sig_features:
                    summary["key_findings"].append(
                        f"Statistically significant features: {len(sig_features)}"
                    )
            ...
            if "stationarity" in results:
                non_stat_count = sum(
                    1
                    for res in results["stationarity"].values()
    ...
    def save_diagnostic_report(
        self, output_path: Union[str, Path], format_type: str = "json"
    ) -> Path:
