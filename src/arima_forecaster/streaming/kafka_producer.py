...
class ForecastMessage(BaseModel):
    ...
    confidence_interval: List[float] = Field(
        ..., description="Intervallo confidenza [lower, upper]"
    )
...
class KafkaForecastProducer:
    ...
    def send_forecast(self, forecast: ForecastMessage) -> bool:
        ...
        try:
            ...
            # Invia asincrono
            future = self.producer.send(topic=self.config.topic, key=key, value=message_data)
    ...
    def _on_send_success(self, record_metadata):
        ...
        logger.debug(
            f"Messaggio inviato: topic={record_metadata.topic}, "
    ...
    def _store_locally(self, forecast: ForecastMessage):
        ...
        try:
            ...
            # Salva come JSON
            filename = (
                f"forecast_{forecast.model_id}_{forecast.timestamp.strftime('%Y%m%d_%H%M%S')}.json"
            )
