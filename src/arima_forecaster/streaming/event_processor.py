...
class EventProcessor:
    ...
    def _register_default_actions(self):
        ...
        def save_to_file(event: ForecastEvent):
            ...
            try:
                ...
                filename = f"event_{event.type.value}_{event.timestamp.strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def submit_forecast_event(
        ...
        event_type = EventType.ANOMALY_DETECTED if anomaly_detected else EventType.FORECAST_GENERATED
    ...
    def submit_error_event(self, model_id: str, error_message: str, error_type: str = "general") -> bool:
            ...
            for rule in applicable_rules:
                if rule.action in self.actions:
                    ...
                    if not success and event.retry_count < event.max_retries:
                        ...
                        logger.info(f"Evento {event.id} riprovato ({event.retry_count}/{event.max_retries})")
                else:
        ...
        while self.is_running:
            try:
                ...
                stats = {
                    ...
                    "success_rate": (self.processed_events / max(self.processed_events + self.failed_events, 1)) * 100,
