...
class AnomalyDetector:
    ...
    def check_anomaly(self, model_id: str, predicted_value: float) -> Dict[str, Any]:
        ...
        if is_anomaly:
            logger.warning(f"Anomalia rilevata per {model_id}: valore={predicted_value}, "
...
class RealtimeForecastService:
    ...
    async def _forecast_scheduler(self):
        ...
        logger.info(f"Forecast scheduler avviato (frequenza: {self.config.update_frequency_seconds}s)")
        ...
        while self.is_running:
            try:
                ...
                logger.debug(f"Batch forecast completato in {elapsed:.2f}s per {len(model_ids)} modelli")
            ...
            def do_forecast():
                if hasattr(model, 'predict'):
                    ...
                    return {
                        ...
                        "lower_ci": [(p * 0.9) for p in predictions.tolist()] if hasattr(predictions, 'tolist') else [predictions * 0.9],
                        "upper_ci": [(p * 1.1) for p in predictions.tolist()] if hasattr(predictions, 'tolist') else [predictions * 1.1]
                    }
