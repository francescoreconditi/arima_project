...
class WebSocketServer:
    ...
    async def start_server(self):
        ...
        try:
            ...
            await asyncio.gather(self.heartbeat_task(), self.message_processor_task())
    ...
    async def process_client_message(self, client_id: str, raw_message: str):
        ...
        try:
            ...
            if message_type == "subscribe":
                ...
                # Risponde a ping
                await self.send_to_client(
                    client_id, {"type": "pong", "timestamp": datetime.now().isoformat(), "data": {}}
                )
    ...
    async def broadcast_forecast_update(self, model_id: str, forecast_data: Dict[str, Any]):
        ...
        message = {
            ...
            "timestamp": datetime.now().isoformat(),
            "data": {"model_id": model_id, "forecast": forecast_data},
        }
        ...
        for client_id, subscription in self.subscriptions.items():
            if (
                not subscription.model_ids or model_id in subscription.model_ids
            ) and "new_forecast" in subscription.update_types:
        ...
        if self.redis_client:
            try:
                self.redis_client.lpush(
                    f"forecast_updates:{model_id}", json.dumps(message, default=str)
                )
    ...
    async def broadcast_anomaly_alert(self, model_id: str, anomaly_data: Dict[str, Any]):
        ...
        for client_id, subscription in self.subscriptions.items():
            if (
                not subscription.model_ids or model_id in subscription.model_ids
            ) and "anomaly_detected" in subscription.update_types:
                target_clients.append(client_id)
    ...
    async def _process_queued_message(self, message: Dict[str, Any]):
        ...
        if message_type == "forecast_update":
            await self.broadcast_forecast_update(message["model_id"], message["data"])
        elif message_type == "anomaly_alert":
            await self.broadcast_anomaly_alert(message["model_id"], message["data"])
