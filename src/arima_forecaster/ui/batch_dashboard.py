...
try:
    from arima_forecaster.automl.batch_processor import BatchForecastProcessor, BatchProgress, BatchTaskResult
...
class BatchForecastDashboard:
    ...
    def render_header(self):
        ...
        st.markdown('<h1 class="main-header">📈 ARIMA AutoML Batch Forecasting</h1>', unsafe_allow_html=True)
        st.markdown("""
        <div class="info-box">
    ...
    def render_sidebar(self):
        ...
        enable_parallel = st.sidebar.checkbox(
            ...
            value=True,
            help="Usa più core CPU per velocizzare"
        )
    ...
    def show_data_preview(self):
        ...
        selected_series = st.selectbox(
            ...
            options=list(st.session_state.uploaded_data.keys())
        )
    ...
    def render_batch_results(self):
        ...
        successful_series = sum(1 for r in st.session_state.batch_results.values() if r.status == 'success')
        failed_series = total_series - successful_series
        avg_time = np.mean([r.training_time for r in st.session_state.batch_results.values() if r.status == 'success'])
        
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("Serie Totali", total_series)
        with col2:
            st.metric("Successi", successful_series, delta=f"{successful_series/total_series:.1%}")
        with col3:
    ...
    def show_results_table(self):
        ...
        for name, result in st.session_state.batch_results.items():
            if result.status == 'success':
                results_data.append({
                    ...
                    'Forecast Medio': f"{np.mean(result.forecast):.2f}" if result.forecast is not None else "N/A",
        ...
        with col2:
            sort_by = st.selectbox(
                "Ordina per:",
                options=['Serie', 'Confidence', 'Accuracy', 'Tempo (s)'],
                index=1
            )
    ...
    def show_results_charts(self):
        ...
        successful_results = {k: v for k, v in st.session_state.batch_results.items() if v.status == 'success'}
        ...
        with tab2:
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(size=10, color=confidence_data, colorscale='Viridis', showscale=True),
                name='Serie'
            ))
            fig.update_layout(
        ...
        with tab3:
            # Forecast comparison
            selected_series = st.selectbox(
                "Seleziona serie per dettaglio forecast:",
