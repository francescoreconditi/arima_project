...
class PerformanceBenchmark:
    ...
    def run_single_benchmark(
        self, config: BenchmarkConfig, dataset: pd.Series, arima_order: Tuple[int, int, int]
        ...
        for rep in range(config.repetitions):
            logger.debug(
                f"Running {config.name} rep {rep + 1}/{config.repetitions} "
    ...
    def run_configuration_comparison(
        ...
        logger.info(
            f"Running configuration comparison on {dataset_name} "
