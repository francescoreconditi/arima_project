...
class FeatureImportanceAnalyzer:
    ...
    def _analyze_correlation(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, feature_name in enumerate(self.feature_names):
                ...
                # Significatività
                significant = (
                    pearson_p < self.config.significance_level
                    and abs(pearson_r) > self.config.correlation_threshold
                )
            ...
            return {
                ...
                "ranking": [
                    {"feature": name, "score": abs(data["pearson_r"])} for name, data in ranking
                ],
                "significant_features": [
                    name for name, data in correlations.items() if data["significant"]
                ],
                "method": "correlation",
            }
    ...
    def _analyze_mutual_information(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            # Ranking
            ranking = sorted(mutual_info.items(), key=lambda x: x[1]["mi_score"], reverse=True)
    ...
    def _analyze_permutation_importance(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            # Ranking
            ranking = sorted(
                permutation_scores.items(), key=lambda x: x[1]["importance_mean"], reverse=True
            )

            return {
                "permutation_importance": permutation_scores,
                "ranking": [
                    {"feature": name, "score": data["importance_mean"]} for name, data in ranking
                ],
                "method": "permutation",
            }
        ...
        try:
            ...
            for i, feature_name in enumerate(self.feature_names):
                ...
                try:
                    ...
                    numerator = np.sum(
                        (clean_feature - feature_mean) * (clean_target - target_mean)
                    )
            ...
            ranking = sorted(test_results.items(), key=lambda x: x[1]["f_statistic"], reverse=True)

            return {
                "univariate_tests": test_results,
                "ranking": [
                    {"feature": name, "score": data["f_statistic"]} for name, data in ranking
                ],
                "significant_features": [
                    name for name, data in test_results.items() if data["significant"]
                ],
                "method": "univariate",
            }
    ...
    def _combine_rankings(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            final_ranking = sorted(
                all_rankings.items(), key=lambda x: x[1]["total_score"], reverse=True
            )
    ...
    def _get_top_features(self, results: Dict[str, Any], top_k: int = 5) -> Dict[str, Any]:
        """Identifica top features più importanti"""
        try:
            if (
                "combined_ranking" not in results
                or "final_ranking" not in results["combined_ranking"]
            ):
    ...
    def _generate_importance_summary(self, results: Dict[str, Any]) -> str:
        ...
        try:
            ...
            summary = (
                f"Analisi Feature Importance ({n_features} features, {n_samples} campioni):\n\n"
            )
            ...
            # Metodi usati
            methods_used = [
                m for m in self.config.methods if m in results and "error" not in results[m]
            ]
            if methods_used:
            ...
            for method in methods_used:
                ...
                if "significant_features" in method_results:
                    ...
                    summary += (
                        f"{method.capitalize()}: {len(sig_features)} features significative\n"
                    )
    ...
    def select_features(
        self, threshold: float = 0.1, max_features: Optional[int] = None
    ) -> List[str]:
        """
        ...
        try:
            ...
            selected = [
                item["feature"] for item in final_ranking if item["combined_score"] >= threshold
            ]
    ...
    def get_feature_insights(self, feature_name: str) -> Dict[str, Any]:
        ...
        insights = {"feature_name": feature_name, "insights": {}, "recommendations": []}
                ...
                if method in self.analysis_results and "error" not in self.analysis_results[method]:
                    ...
                    if method == "correlation":
                        ...
                        insights["insights"]["mutual_info"] = {
                            ...
                            "interpretation": self._interpret_mutual_info(
                                mi_data.get("normalized_score", 0)
                            ),
                        }

            # Raccomandazioni
            insights["recommendations"] = self._generate_feature_recommendations(
                feature_name, insights["insights"]
            )
    ...
    def _generate_feature_recommendations(
        self, feature_name: str, insights: Dict[str, Any]
    ) -> List[str]:
...
if __name__ == "__main__":
    ...
    if selected:
        ...
        print(
            f"Insights per {selected[0]}: {len(insights.get('recommendations', []))} raccomandazioni"
        )
