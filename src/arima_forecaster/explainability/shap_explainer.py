...
class SHAPExplainer:
    ...
    def fit(self, model: Any, background_data: np.ndarray, feature_names: Optional[List[str]] = None):
        """
        ...
        try:
            ...
            self.feature_names = feature_names or self.config.feature_names or \
    ...
    def _determine_explainer_type(self, model: Any) -> str:
        ...
        if any(tree_model in model_type for tree_model in ["random", "forest", "tree", "xgb", "lgb"]):
            return "tree"
    ...
    def _model_predict_wrapper(self, X: np.ndarray) -> np.ndarray:
        ...
        try:
            if hasattr(self.model, 'predict'):
                ...
                predictions = []
                for row in X:
                    pred = self.model.forecast(steps=1, exog_variables=row if len(row) > 0 else None)
    ...
    def explain_instance(
        ...
        try:
            ...
            shap_values = self.explainer.shap_values(
                X=instance,
                nsamples=self.config.max_evals
            )
    ...
    def _calculate_confidence_factors(
        ...
        consistency = 1.0 - min(abs(prediction - expected_prediction) / (abs(prediction) + 1e-10), 1.0)
    ...
    def _create_fallback_explanation(self, instance: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            "feature_contributions": {},
            "feature_ranking": [],
            "confidence_factors": {"overall": 0.0, "stability": 0.0, "consistency": 0.0, "coverage": 0.0},
...
def explain_arima_forecast(
    ...
    explanation = explainer.explain_instance(
        instance=forecast_instance,
        cache_key="arima_forecast"
    )
