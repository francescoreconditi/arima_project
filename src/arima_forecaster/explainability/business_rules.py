...
class BusinessRulesEngine:
    ...
    def _initialize_default_rules(self):
        ...
        default_rules = [
            ...
            Rule(
                ...
                action_params={
                    "new_value": "context.max_capacity",
                    "message": "Forecast limitato alla capacità massima",
                },
                priority=1,
            ),
            Rule(
                ...
                action=RuleAction.MODIFY,
                action_params={
                    "new_value": "context.min_capacity",
                    "message": "Forecast alzato al minimo operativo",
                },
                priority=2,
            ),
            ...
            Rule(
                ...
                action_params={
                    "message": "Forecast significativamente superiore alla media storica"
                },
                priority=3,
            ),
            Rule(
            ...
            Rule(
                ...
                action=RuleAction.MODIFY,
                action_params={
                    "new_value": "context.budget_limit",
                    "message": "Forecast limitato dal budget disponibile",
                },
                priority=2,
            ),
            ...
            Rule(
                ...
                action_params={
                    "transform": "round",
                    "message": "Forecast arrotondato a unità intere",
                },
                priority=5,
            ),
        ]
    ...
    def apply_rules(
        self, forecast_value: float, context: BusinessContext, rule_ids: Optional[List[str]] = None
    ...
    def _apply_single_rule(
        self, rule: Rule, current_value: float, context: BusinessContext
    ) -> RuleResult:
    ...
    def _execute_rule_action(
        ...
        try:
            if rule.action == RuleAction.ACCEPT:
                ...
                new_value = self._calculate_modified_value(
                    current_value, action_params, eval_context
                )
    ...
    def _calculate_modified_value(
        self, current_value: float, action_params: Dict[str, Any], eval_context: Dict[str, Any]
    ) -> float:
    ...
    def get_rule_explanations(
        self, forecast_value: float, context: BusinessContext, results: List[RuleResult]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            explanation = {
                ...
                "final_value": modifications[-1].modified_value
                if modifications
                else forecast_value,
    ...
    def _generate_business_rationale(
        self, applied_rules: List[RuleResult], context: BusinessContext
    ) -> str:
        ...
        constraint_rules = [
            r for r in applied_rules if self.rules[r.rule_id].rule_type == RuleType.CONSTRAINT
        ]
        adjustment_rules = [
            r for r in applied_rules if self.rules[r.rule_id].rule_type == RuleType.ADJUSTMENT
        ]
        validation_rules = [
            r for r in applied_rules if self.rules[r.rule_id].rule_type == RuleType.VALIDATION
        ]

        if constraint_rules:
            rationale_parts.append(
                f"Applicati {len(constraint_rules)} vincoli operativi per rispettare limiti di capacità e budget"
            )

        if adjustment_rules:
            rationale_parts.append(
                f"Applicate {len(adjustment_rules)} regole di aggiustamento per contesto business specifico"
            )

        if validation_rules:
            rationale_parts.append(
                f"Applicate {len(validation_rules)} validazioni per garantire coerenza logica"
            )
    ...
    def _check_compliance_status(self, results: List[RuleResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "compliance_score": len(
                [r for r in results if r.applied and r.action_taken != RuleAction.REJECT]
            )
            / max(len(results), 1),
        }

    def _generate_rule_recommendations(
        self, results: List[RuleResult], context: BusinessContext
    ) -> List[str]:
        ...
        if alerts:
            recommendations.append(
                f"Monitorare {len(alerts)} alert generati per potenziali anomalie"
            )
        ...
        if len(modifications) > 3:
            recommendations.append(
                "Numero elevato di modifiche - considerare revisione regole o modello"
            )

        # Context-specific recommendations
        if context.max_capacity and any("capacità" in r.message.lower() for r in results):
            recommendations.append(
                "Valutare espansione capacità se forecast continua a essere limitato"
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        recent_results = [
            r for r in self.rule_history if r.timestamp > datetime.now() - timedelta(days=7)
        ]
...
def apply_business_rules_to_forecast(
    forecast_value: float, context: BusinessContext, rules_engine: BusinessRulesEngine = None
) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "modification_percentage": ((final_value - forecast_value) / forecast_value * 100)
        if forecast_value != 0
        else 0,
