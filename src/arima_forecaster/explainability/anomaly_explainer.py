...
class AnomalyExplainer:
    ...
    def add_historical_data(
        self, model_id: str, data: pd.Series, metadata: Optional[Dict[str, Any]] = None
    ):
    ...
    def explain_anomaly(
        ...
        try:
            ...
            if anomaly_score is None:
                anomaly_score = self._estimate_anomaly_score(
                    model_id, predicted_value, actual_value
                )
            ...
            # Calcola deviazione
            deviation = self._calculate_deviation_magnitude(
                predicted_value, actual_value, expected_range
            )
    ...
    def _estimate_anomaly_score(
        self, model_id: str, predicted_value: float, actual_value: Optional[float]
    ) -> float:
    ...
    def _determine_severity(
        self, anomaly_score: float, predicted_value: float, actual_value: Optional[float]
    ) -> AnomalySeverity:
    ...
    def _classify_anomaly_type(
        ...
        try:
            ...
            if context_data and "expected_seasonal" in context_data:
                ...
                seasonal_dev = abs(predicted_value - expected_seasonal) / max(
                    abs(expected_seasonal), 1
                )
                if seasonal_dev > 0.3:
    ...
    def _calculate_expected_range(
        self, model_id: str, predicted_value: float, context_data: Optional[Dict[str, Any]]
    ) -> Tuple[float, float]:
    ...
    def _find_contributing_factors(
        ...
        try:
                        ...
                        factors.append(
                            {
                                ...
                                "contribution_score": min(
                                    abs(recent_mean - overall_mean) / hist_stats["std"], 1.0
                                ),
            ...
            return [
                {
                    "factor": "Analisi fattori non disponibile",
    ...
    def _find_similar_historical_cases(
        ...
        try:
            ...
            similar_values = hist_data[abs(hist_data - predicted_value) <= threshold]
        ...
        try:
            ...
            if severity == AnomalySeverity.CRITICAL:
                ...
                recommendations.append(
                    "Considerar sospendere processi automatici fino a risoluzione"
                )
        ...
        try:
            ...
            if contributing_factors:
                avg_contribution = np.mean(
                    [f.get("contribution_score", 0) for f in contributing_factors]
                )
...
if __name__ == "__main__":
    ...
    explanation = explainer.explain_anomaly(
        model_id="test_model", predicted_value=anomaly_value, actual_value=None, anomaly_score=0.85
    )
