    ...
    def _create_model_training_section(self):
        ...
        with cols[0]:
            model_type = st.selectbox(
                "Model Type", ["ARIMA", "SARIMA", "Prophet"], help="Choose forecasting model type"
            )
    ...
    def _create_whatif_section(self):
        ...
        if "scenario_results" in st.session_state and st.session_state.scenario_results:
            ...
            fig = self.scenario_simulator.create_scenario_visualization(
                st.session_state.base_forecast, scenario_data["forecast"], scenario_data["results"]
            )
        ...
        with st.spinner(f"Training {model_type} model..."):
            try:
                ...
                if "date" in self.data.columns and "value" in self.data.columns:
                    series = pd.Series(
                        self.data["value"].values, index=pd.to_datetime(self.data["date"])
                    )
                else:
                    # Assume first column is date, second is value
                    series = pd.Series(
                        self.data.iloc[:, 1].values, index=pd.to_datetime(self.data.iloc[:, 0])
                    )
                ...
                if model_type == "ARIMA":
                    ...
                    if auto_params:
                        selector = SARIMAModelSelector(
                            max_models=10
                        )  # Pass max_models to constructor
                        selector.search(series)  # Search for best parameters
                        self.model = selector.get_best_model()  # Get the trained model
                    else:
                        self.model = SARIMAForecaster(order=(1, 1, 1), seasonal_order=(1, 1, 1, 12))
                        self.model.fit(series)
        ...
        if isinstance(forecast_data, dict):
            ...
            if "forecast" in forecast_data:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        if hasattr(forecast_series, "index")
                        else list(range(len(forecast_series))),
                        y=forecast_series.values
                        if hasattr(forecast_series, "values")
            ...
            if "confidence_intervals" in forecast_data:
                ...
                if "lower" in ci_data and "upper" in ci_data:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=upper_ci.index
                            if hasattr(upper_ci, "index")
                            else list(range(len(upper_ci))),
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=lower_ci.index
                            if hasattr(lower_ci, "index")
                            else list(range(len(lower_ci))),
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    else list(range(len(forecast_data))),
        ...
        with st.spinner("Generating procurement report..."):
            try:
                ...
                if full_report:
                    ...
                    filename = (
                        f"Procurement_Report_Full_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
                    )
                else:
