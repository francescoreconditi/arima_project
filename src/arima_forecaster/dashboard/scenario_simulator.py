...
class WhatIfScenarioSimulator:
    ...
    def create_scenario_ui(self) -> ScenarioParameters:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(
            ["📈 Demand Drivers", "🏭 Supply Chain", "💰 Economics", "⚡ External Shocks"]
        )
    ...
    def _get_predefined_scenario(self, scenario_type: ScenarioType) -> ScenarioParameters:
        ...
        scenarios = {
            ...
            ScenarioType.SUPPLIER_ISSUE: ScenarioParameters(
                supplier_reliability=70.0, lead_time_change=14, capacity_limit=60.0
            ),
            ScenarioType.SEASONAL_PEAK: ScenarioParameters(
                seasonality_factor=2.2, marketing_boost=50.0, capacity_limit=120.0
            ),
            ScenarioType.PRICE_WAR: ScenarioParameters(
                price_change=-25.0, competitor_impact=-30.0, marketing_boost=80.0
            ),
            ScenarioType.NEW_COMPETITOR: ScenarioParameters(
                competitor_impact=-25.0, marketing_boost=40.0, price_change=-15.0
            ),
    ...
    def run_scenario_simulation(
        self, params: ScenarioParameters, base_forecast: pd.Series, base_metrics: Dict[str, Any]
        ...
        results = self._calculate_scenario_impact(
            base_forecast, scenario_forecast, base_metrics, params, cost_multiplier
        )
    ...
    def _generate_recommendations(
        self, params: ScenarioParameters, revenue_change_pct: float, service_risk: float
    ) -> List[str]:
    ...
    def create_scenario_visualization(
        self, base_forecast: pd.Series, scenario_forecast: pd.Series, results: ScenarioResults
    ) -> go.Figure:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                if results.revenue_change_pct > 0
        ...
        cumulative_scenario = (
            scenario_forecast.cumsum() * 150 * (1 + results.revenue_change_pct / 100)
        )
        ...
        fig.add_trace(
            go.Bar(x=timeline_months, y=roi_progression, name="ROI %", marker_color="lightblue"),
            row=2,
            col=2,
        )
    ...
    def display_scenario_results(self, results: ScenarioResults):
        ...
        with col4:
            st.metric("3M ROI", f"{results.roi_3months:+.1f}%", help="Return on Investment 3 mesi")
        ...
        with col3:
            break_even_text = (
                f"{results.break_even_days} giorni" if results.break_even_days < 365 else "N/A"
            )
