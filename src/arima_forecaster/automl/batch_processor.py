...
class BatchForecastProcessor:
    ...
    def _process_serial(self, tasks: Dict[str, Any], automl_config: Dict, forecast_steps: int):
        """Processamento seriale per debug"""
        for task_id, data in tasks.items():
            if self.verbose:
                print(f"[TASK] Processing {task_id}...")

            result = self._process_single_task(task_id, data, automl_config, forecast_steps)
    ...
    def _process_parallel(self, tasks: Dict[str, Any], automl_config: Dict, forecast_steps: int):
        """Processamento parallelo con thread/process pool"""

        executor_class = (
            ProcessPoolExecutor if self.parallel_mode == "process" else ThreadPoolExecutor
        )
        ...
        with executor_class(max_workers=self._get_worker_count()) as executor:
            ...
            future_to_task = {
                executor.submit(
                    self._process_single_task, task_id, data, automl_config, forecast_steps
                ): task_id
                for task_id, data in tasks.items()
            }
    ...
    def _process_single_task(
        self, task_id: str, data: Any, automl_config: Optional[Dict], forecast_steps: int
    ) -> BatchTaskResult:
        """
    ...
    def _update_progress(self):
        ...
        if self.verbose and self.progress.completed_tasks % 10 == 0:
            ...
            print(
                f"[PROGRESS] {pct:.1f}% ({self.progress.completed_tasks}/{self.progress.total_tasks}) | "
    ...
    def _load_cached_results(self, batch_id: str):
        ...
        if cache_file.exists():
            try:
                ...
                # Update progress
                completed = sum(
                    1 for r in self.results.values() if r.status == BatchStatus.COMPLETED
                )
                failed = sum(1 for r in self.results.values() if r.status == BatchStatus.FAILED)
    ...
    def get_batch_summary(self) -> Dict[str, Any]:
        ...
        completed_results = [r for r in self.results.values() if r.status == BatchStatus.COMPLETED]
        failed_results = [r for r in self.results.values() if r.status == BatchStatus.FAILED]
        ...
        # Timing stats
        processing_times = [
            r.processing_time for r in completed_results if r.processing_time is not None
        ]
        ...
        return {
            ...
            "avg_confidence": np.mean(
                [r.confidence for r in completed_results if r.confidence is not None]
            )
    ...
    def retry_failed_tasks(
        ...
        failed_datasets = {
            task_id: datasets[task_id] for task_id in failed_task_ids if task_id in datasets
        }
...
def process_excel_portfolio(
    excel_file_path: str, sheet_name: str = 0, **csv_kwargs
) -> Dict[str, BatchTaskResult]:
    ...
    for i in range(10):
        ...
        if i % 3 == 0:
            ...
            data = np.random.choice(
                [0, 0, 0, 1, 2, 3], size=200, p=[0.7, 0.1, 0.05, 0.1, 0.03, 0.02]
            )  # intermittent

        test_data[f"product_{i:02d}"] = data

    # Test batch processing
    processor = BatchForecastProcessor(max_workers=4, parallel_mode="thread", verbose=True)
