...
class SeriesPatternDetector:
    ...
    def analyze_series(self, data: Union[pd.Series, np.ndarray]) -> PatternAnalysis:
        ...
        characteristics["cv"] = (
            characteristics["std"] / characteristics["mean"] if characteristics["mean"] != 0 else 0
        )
        ...
        data_type, recommendations, confidence, explanation = self._classify_pattern(
            characteristics
        )
    ...
    def _detect_seasonality(self, values: np.ndarray) -> Dict[str, float]:
        ...
        if len(values) < 24:  # Troppo corto
            return {"seasonality_strength": 0.0, "dominant_period": 0, "seasonal_significance": 1.0}
    ...
    def _analyze_volatility(self, values: np.ndarray) -> Dict[str, float]:
        ...
        return {"volatility": volatility, "outlier_ratio": outlier_ratio}
    ...
    def _classify_pattern(
        self, char: Dict[str, float]
    ) -> Tuple[DataType, List[ModelType], float, str]:
        ...
        if char["seasonality_strength"] > 0.6 and char["dominant_period"] > 0:
            if char["trend_r2"] > 0.3:
                models = (
                    [ModelType.PROPHET, ModelType.SARIMA] if _has_prophet else [ModelType.SARIMA]
                )
                return (
                    DataType.SEASONAL,
                    models,
...
class AutoForecastSelector:
    ...
    def fit(
        ...
        try:
            clean_data, metadata = self.preprocessor.preprocess_pipeline(
                data, handle_missing="interpolate", test_stationarity=True
            )
        ...
        self.model_results_ = self._train_candidate_models(train_data, test_data, exog)
    ...
    def _train_candidate_models(
        self, train_data: pd.Series, test_data: pd.Series, exog: Optional[np.ndarray] = None
    ) -> List[ModelResult]:
    ...
    def _train_single_model(
        ...
        try:
            ...
            if model_type == ModelType.ARIMA:
                ...
                if season_period == 0:
                    season_period = 12
                model = SARIMAForecaster(order=(1, 1, 1), seasonal_order=(1, 1, 1, season_period))
                ...
                from arima_forecaster.core.intermittent_model import (
                    IntermittentConfig,
                    IntermittentMethod,
                )
                ...
                model = ProphetForecaster(
                    yearly_seasonality=True, weekly_seasonality=True, country_holidays="IT"
                )
            ...
            if model_type == ModelType.INTERMITTENT:
                ...
                performance_metrics = {"mae": mae, "mape": 100 * mae / max(test_data.mean(), 1)}

            # Calculate confidence based on multiple factors
            confidence = self._calculate_model_confidence(accuracy_score, training_time, model_type)
    ...
    def _calculate_model_confidence(
        self, accuracy_score: float, training_time: float, model_type: ModelType
    ) -> float:
    ...
    def _generate_explanation(self) -> AutoMLExplanation:
        ...
        if pattern.data_type == DataType.INTERMITTENT:
            why_chosen += (
                f"domanda intermittente (zeri: {pattern.characteristics['zero_ratio']:.1%})"
            )
        ...
        if best.model_type == ModelType.INTERMITTENT:
            business_rec = (
                f"Gestire come spare part. Safety stock raccomandato basato su service level 95%."
            )
