...
class HyperparameterTuner:
        ...
        **optimizer_kwargs,
        ...
        for iteration in range(max_iterations):
            ...
            try:
                ...
                improvement = (
                    (best_score - current_score) / best_score if best_score != float("inf") else 0
                )
    ...
    def forecast_ensemble(
        self, steps: int, confidence_level: float = 0.95, method: str = "weighted"
    ) -> Dict[str, Any]:
        """
        ...
        if method == "median":
            ...
            ensemble_std = np.sqrt(
                np.average(
                    (forecasts_array - ensemble_forecast) ** 2, axis=0, weights=weights_array
                )
            )
    ...
    def _evaluate_all_metrics(
        self, model, data: Union[pd.Series, pd.DataFrame]
    ) -> Dict[str, float]:
            ...
            if any(metric in self.objective_metrics for metric in ["mse", "mae", "mape"]):
                if isinstance(data, pd.Series):
                    ...
                    if hasattr(model, "order"):  # ARIMA/SARIMA
                        ...
                        temp_model = type(model)(
                            maxlags=getattr(model, "maxlags", None), ic=getattr(model, "ic", "aic")
                        )
                    ...
                    if "mse" in self.objective_metrics:
                        scores["mse"] = np.mean(
                            (forecast[: len(test_data)] - test_data[: len(forecast)]) ** 2
                        )
                    if "mae" in self.objective_metrics:
                        scores["mae"] = np.mean(
                            np.abs(forecast[: len(test_data)] - test_data[: len(forecast)])
                        )
                    if "mape" in self.objective_metrics:
                        scores["mape"] = (
                            np.mean(
                                np.abs(
                                    (test_data[: len(forecast)] - forecast[: len(test_data)])
                                    / test_data[: len(forecast)]
                                )
                            )
                            * 100
                        )
    ...
    def _is_diverse(
        self, params: Dict[str, Any], seen_params: List[Dict[str, Any]], threshold: float
    ) -> bool:
    ...
    def _calculate_ensemble_score(
        self, ensemble_models: List[Dict[str, Any]], data: Union[pd.Series, pd.DataFrame]
    ) -> float:
