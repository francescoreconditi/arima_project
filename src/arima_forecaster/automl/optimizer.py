...
class BaseOptimizer:
    ...
    def _time_series_split(
        self, series: pd.Series, n_splits: int
    ...
    def _evaluate_model_cv(
        self, model_class: type, params: Dict[str, Any], series: pd.Series
    ) -> float:
        """
                ...
                try:
                    ...
                    if self.objective_metric in ["aic", "bic", "hqic"]:
                        ...
                        if self.objective_metric == "mse":
                            ...
                            score = (
                                np.mean(np.abs((test_aligned - forecast_aligned) / test_aligned))
                                * 100
                            )
                        else:
                            score = float("inf")
...
class ARIMAOptimizer(BaseOptimizer):
    ...
    def optimize_optuna(
        self, series: pd.Series, n_trials: int = 100, timeout: Optional[int] = None
    ) -> Dict[str, Any]:
        """
    ...
    def optimize_hyperopt(self, series: pd.Series, max_evals: int = 100) -> Dict[str, Any]:
        """
        ...
        self.best_params = {"order": (int(best["p"]), int(best["d"]), int(best["q"]))}
    ...
    def optimize_skopt(self, series: pd.Series, n_calls: int = 100) -> Dict[str, Any]:
        """
        ...
        if not SCIKIT_OPT_AVAILABLE:
            raise ImportError(
                "scikit-optimize is not installed. Install with: pip install scikit-optimize"
            )
        ...
        result = gp_minimize(
            func=objective, dimensions=dimensions, n_calls=n_calls, random_state=self.random_state
        )
...
class VAROptimizer(BaseOptimizer):
    ...
    def __init__(
        self, maxlags_range: Tuple[int, int] = (1, 10), ic_options: List[str] = None, **kwargs
    ):
        """
    ...
    def optimize_optuna(
        self, data: pd.DataFrame, n_trials: int = 50, timeout: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        self.best_params = {"maxlags": study.best_params["maxlags"], "ic": study.best_params["ic"]}
        self.best_score = study.best_value
...
def optimize_model(
    model_type: str, data: Union[pd.Series, pd.DataFrame], optimizer_type: str = "optuna", **kwargs
) -> Dict[str, Any]:
    """
