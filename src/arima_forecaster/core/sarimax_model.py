...
from ..utils.exceptions import ForecastError, ModelTrainingError
...
class SARIMAXForecaster:
    ...
    def fit(
        ...
        **fit_kwargs,
        ...
        try:
            exog_info = (
                f" con {exog.shape[1] if exog is not None else 0} variabili esogene"
                if exog is not None
    ...
    def forecast(
        ...
        try:
            if self.fitted_model is None:
                raise ForecastError(
                    "Il modello SARIMAX deve essere addestrato prima della previsione"
                )
            ...
            forecast_result = self.fitted_model.get_forecast(
                steps=steps, exog=exog_future, alpha=alpha
            )
    ...
    def predict(
        ...
        try:
            if self.fitted_model is None:
                raise ForecastError(
                    "Il modello SARIMAX deve essere addestrato prima della predizione"
                )
            ...
            for name in self.exog_names:
                if name in self.fitted_model.params.index:
                    exog_params[name] = {
                        ...
                        "pvalue": float(self.fitted_model.pvalues[name])
                        if hasattr(self.fitted_model, "pvalues")
                        else None,
                        "tvalue": float(self.fitted_model.tvalues[name])
                        if hasattr(self.fitted_model, "tvalues")
                        else None,
                    }
    ...
    def get_seasonal_decomposition(self) -> Dict[str, pd.Series]:
        ...
        if self.fitted_model is None:
            raise ForecastError(
                "Il modello SARIMAX deve essere addestrato prima della decomposizione"
            )
    ...
    def get_exog_importance(self) -> pd.DataFrame:
        ...
        if self.fitted_model is None:
            raise ForecastError(
                "Il modello SARIMAX deve essere addestrato per analizzare le variabili esogene"
            )
        ...
        try:
            ...
            for name in self.exog_names:
                if name in self.fitted_model.params.index:
                    ...
                    pvalue = (
                        float(self.fitted_model.pvalues[name])
                        if hasattr(self.fitted_model, "pvalues")
                        else None
                    )
                    tvalue = (
                        float(self.fitted_model.tvalues[name])
                        if hasattr(self.fitted_model, "tvalues")
                        else None
                    )
    ...
    def _validate_exog(self, series: pd.Series, exog: pd.DataFrame) -> None:
        ...
        if non_numeric_cols:
            self.logger.warning(
                f"Variabili esogene non numeriche (potrebbero causare problemi): {non_numeric_cols}"
            )
    ...
    def generate_report(
        ...
        try:
            ...
            if report_title is None:
                exog_info = (
                    f" con {len(self.exog_names)} variabili esogene" if self.exog_names else ""
                )
                report_title = (
                    f"Analisi Modello SARIMAX{self.order}x{self.seasonal_order}{exog_info}"
                )
                ...
                try:
                    ...
                    if not exog_importance.empty:
                        model_results["exog_analysis"] = {
                            ...
                            "significant_vars": exog_importance[
                                exog_importance["significant"] == True
                            ]["variable"].tolist(),
                            "n_significant": int(exog_importance["significant"].sum())
                            if "significant" in exog_importance
                            else 0,
                        }
                except Exception as e:
                    self.logger.warning(
                        f"Non è stato possibile includere l'analisi delle variabili esogene: {e}"
                    )
            ...
            if include_forecast:
                try:
                    ...
                    # Altrimenti genera nuovo forecast se possibile
                        ...
                        self.logger.warning(
                            "Forecast nel report non generato: SARIMAX richiede variabili esogene future"
                        )
                        model_results["forecast_note"] = (
                            "Forecast non disponibile: necessarie variabili esogene future"
                        )
