...
class SARIMAModelSelector:
    ...
    def search(
        ...
        try:
            self.logger.info(f"Avvio selezione modello SARIMA usando {self.information_criterion.upper()}")
            ...
            if self.results:
                ...
                self.logger.info(f"Miglior modello SARIMA: {self.best_order}x{self.best_seasonal_order}")
    ...
    def _fit_models_sequential(
        ...
        iterator = tqdm(param_combinations, desc="Test modelli SARIMA") if verbose else param_combinations
