...
class ProphetForecaster:
    ...
    def _initialize_model(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Prophet model initialized with {self.country_holidays or 'no'} country holidays"
            )
    ...
    def _get_country_holidays(self, country: str) -> pd.DataFrame:
        ...
        try:
            ...
            holidays_df = make_holidays_df(year_list=list(years), country=country)
    ...
    def add_seasonality(
        ...
        try:
            ...
            logger.info(
                f"Added seasonality: {name} (period={period}, fourier_order={fourier_order})"
            )
    ...
    def fit(
        self, series: pd.Series, exog: Optional[pd.DataFrame] = None, validate_input: bool = True
    ) -> "ProphetForecaster":
        """
            ...
            if exog is not None:
                ...
                if missing_regressors:
                    raise ModelTrainingError(
                        f"Missing regressors in exog data: {missing_regressors}"
                    )
    ...
    def _make_future_dataframe(
        ...
        if self._regressors:
            if exog_future is None:
                raise ForecastError(f"Future values required for regressors: {self._regressors}")
    ...
    def predict(self, start: Optional[int] = None, end: Optional[int] = None) -> pd.Series:
        ...
        try:
            ...
            if self.forecast_result is not None:
                ...
                return pd.Series(
                    fitted_result["yhat"].values, index=self.last_series.index, name="fitted"
                )
            else:
                ...
                return pd.Series(
                    forecast_df["yhat"].values, index=self.last_series.index, name="fitted"
                )
        ...
        if self.last_series is not None:
            try:
                ...
                metrics = {
                    "mae": mae,
                    "rmse": np.sqrt(mse),
                    "mape": np.mean(np.abs((self.last_series - fitted_values) / self.last_series))
                    * 100,
                }
            except Exception:
        ...
        try:
                ...
                if hasattr(self, "forecast_result") and self.forecast_result is not None:
                    ...
                    try:
                        metrics = evaluator.calculate_forecast_metrics(
                            actual=self.last_series, predicted=fitted_values
                        )
            ...
            if include_forecast:
                try:
                    ...
                    logger.warning(
                        f"Non è stato possibile generare la previsione per il report: {e}"
                    )
