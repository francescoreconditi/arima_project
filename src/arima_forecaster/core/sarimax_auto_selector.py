...
class SARIMAXAutoSelector(SARIMAForecaster):
    ...
    def fit_with_exog(
        self, series: pd.Series, exog: pd.DataFrame, validate_input: bool = True, **fit_kwargs
    ) -> "SARIMAXAutoSelector":
        """
        ...
        try:
            ...
            self.logger.info(
                f"Feature engineering: {len(exog.columns)} → {len(exog_engineered.columns)} feature"
            )
            ...
            self.logger.info(
                f"Feature selection: {len(exog_processed.columns)} → {len(exog_selected.columns)} feature"
            )

            # Step 4: Verifica allineamento finale prima del training
            self.logger.info(f"Verifica allineamento finale:")
            self.logger.info(
                f"Serie: {len(series_aligned)} obs, index: {series_aligned.index[0]} to {series_aligned.index[-1]}"
            )
            self.logger.info(
                f"Exog: {len(exog_selected)} obs, index: {exog_selected.index[0]} to {exog_selected.index[-1]}"
            )
            self.logger.info(
                f"Indici allineati: {series_aligned.index.equals(exog_selected.index)}"
            )
            ...
            if not series_aligned.index.equals(exog_selected.index):
                ...
                self.logger.error(
                    f"Serie index dtype: {series_aligned.index.dtype}, Exog index dtype: {exog_selected.index.dtype}"
                )
        ...
        try:
            ...
            if exog is not None:
                ...
                if self.selected_features:
                    self.logger.warning(
                        "Creazione exog nulle per forecast - modello addestrato con exog"
                    )
            ...
            forecast_result = self.fitted_model.get_forecast(
                steps=steps, exog=exog_processed, alpha=alpha
            )
    ...
    def _engineer_features(
        self, exog: pd.DataFrame, series: pd.Series
    ...
    def _preprocess_features(self, exog: pd.DataFrame) -> pd.DataFrame:
        ...
        self.preprocessor = ExogenousPreprocessor(method=method, handle_outliers=True)
    ...
    def _stepwise_selection(
        ...
        for iteration in range(max_iterations):
            ...
            if best_feature and best_aic_iter < best_aic:
                ...
                self.logger.debug(
                    f"Stepwise iteration {iteration + 1}: aggiunta '{best_feature}' (AIC: {best_aic:.2f})"
                )
            else:
    ...
    def _lasso_selection(
        ...
        self.logger.debug(
            f"Lasso selection: {len(selected_features)} feature selezionate (alpha: {lasso.alpha_:.6f})"
        )
    ...
    def _elastic_net_selection(
        ...
        self.logger.debug(
            f"Elastic Net selection: {len(selected_features)} feature (alpha: {elastic.alpha_:.6f}, l1_ratio: {elastic.l1_ratio_:.3f})"
        )
    ...
    def _f_test_selection(
        ...
        self.logger.debug(
            f"F-test selection: {len(selected_features)} feature con maggiore F-statistic"
        )
    ...
    def _calculate_feature_importance(self, exog_selected: pd.DataFrame) -> None:
        ...
        try:
            ...
            if hasattr(self.fitted_model, "params"):
                ...
                self.logger.debug(
                    f"Feature importance calcolata per {len(self.feature_importance)} feature"
                )
    ...
    def _prepare_forecast_exog(self, exog: pd.DataFrame, steps: int) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if available_features:
                ...
                self.logger.info(
                    f"Preparazione exog forecast completata: {len(exog_final)} righe, {len(available_features)} features"
                )
                return exog_final
            else:
                return None
    ...
    def _validate_inputs(self, series: pd.Series, exog: pd.DataFrame) -> None:
        ...
        if not series.index.equals(exog.index):
            self.logger.warning(
                "Indici serie e exog non perfettamente allineati - applicando allineamento automatico"
            )
            raise ModelTrainingError(
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.preprocessing_method not in valid_preprocessing_methods:
            raise ValueError(
                f"preprocessing_method deve essere uno di: {valid_preprocessing_methods}"
            )
        ...
        if not all(method in valid_feature_engineering for method in self.feature_engineering):
            raise ValueError(
                f"feature_engineering deve contenere elementi di: {valid_feature_engineering}"
            )
    ...
    def _update_training_metadata(
        self, series: pd.Series, exog_original: pd.DataFrame, exog_selected: pd.DataFrame
    ) -> None:
    ...
    def get_feature_analysis(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "selection_summary": {
                ...
                "selection_ratio": len(self.selected_features)
                / (len(self.original_features) + len(self.engineered_features)),
            },
            "feature_details": {
                "original_features": self.original_features,
                "engineered_features": self.engineered_features,
