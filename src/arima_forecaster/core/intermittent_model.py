...
class IntermittentForecaster:
    ...
    def predict_intervals(self, steps: int = 1, confidence: float = 0.95) -> Dict[str, np.ndarray]:
        """
    ...
    def calculate_safety_stock(self, lead_time: int, service_level: float = 0.95) -> float:
        """
    ...
    def calculate_reorder_point(self, lead_time: int, service_level: float = 0.95) -> float:
        """
    ...
    def __repr__(self) -> str:
        if self.is_fitted:
            return (
                f"IntermittentForecaster(method={self.config.method.value}, "
