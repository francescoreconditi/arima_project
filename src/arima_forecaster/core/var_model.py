...
class VARForecaster:
    ...
    def fit(
        self, data: pd.DataFrame, validate_input: bool = True, trend: str = "c", **fit_kwargs
    ) -> "VARForecaster":
        """
        ...
        try:
            self.logger.info(
                f"Addestramento modello VAR su {data.shape[0]} osservazioni con {data.shape[1]} variabili"
            )
            ...
            if self.maxlags is None:
                ...
                self.logger.info(
                    f"Ordine lag auto-selezionato: {self.selected_lag} usando {self.ic.upper()}"
                )
            else:
            ...
            # Addestra il modello
            self.fitted_model = self.model.fit(maxlags=self.selected_lag, trend=trend, **fit_kwargs)
    ...
    def forecast(self, steps: int, alpha: float = 0.05) -> Dict[str, pd.DataFrame]:
        """
        ...
        try:
            ...
            forecast_result = self.fitted_model.forecast(
                y=self.training_data.values[-self.fitted_model.k_ar :], steps=steps
            )
            ...
            forecast_df = pd.DataFrame(
                forecast_result, index=forecast_index, columns=self.training_data.columns
            )

            # Ottieni intervalli di confidenza delle previsioni
            try:
                conf_int = self.fitted_model.forecast_interval(
                    y=self.training_data.values[-self.fitted_model.k_ar :], steps=steps, alpha=alpha
                )
                ...
                if isinstance(conf_int, tuple) and len(conf_int) == 2:
                    ...
                    lower_bounds = pd.DataFrame(
                        conf_int[0], index=forecast_index, columns=self.training_data.columns
                    )
                    upper_bounds = pd.DataFrame(
                        conf_int[1], index=forecast_index, columns=self.training_data.columns
                    )
    ...
    def impulse_response(
        ...
        try:
            if self.fitted_model is None:
                raise ForecastError(
                    "Il modello VAR deve essere addestrato prima dell'analisi della risposta agli impulsi"
                )
            ...
            columns = pd.MultiIndex.from_product(
                [variables, variables], names=["impulse", "response"]
            )
            ...
            irf_df = pd.DataFrame(reshaped_data, columns=columns, index=range(periods))
    ...
    def forecast_error_variance_decomposition(
        self, periods: int = 20, normalize: bool = True
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            variables = self.training_data.columns
            columns = pd.MultiIndex.from_product(
                [variables, variables], names=["variable", "shock_source"]
            )
            ...
            fevd_df = pd.DataFrame(reshaped_data, columns=columns, index=range(1, periods + 1))
    ...
    def granger_causality(
        ...
        try:
            if self.fitted_model is None:
                raise ForecastError(
                    "Il modello VAR deve essere addestrato prima del test di causalità di Granger"
                )

            if causing_variables is None:
                causing_variables = [
                    col for col in self.training_data.columns if col != caused_variable
                ]
            ...
            for causing_var in causing_variables:
                test_result = self.fitted_model.test_causality(
                    caused=caused_variable, causing=causing_var, kind="f"
                )
    ...
    def cointegration_test(self, test_type: str = "johansen") -> Dict[str, Any]:
        ...
        try:
            if self.fitted_model is None:
                raise ForecastError(
                    "Il modello VAR deve essere addestrato prima del test di cointegrazione"
                )
    ...
    def _validate_data(self, data: pd.DataFrame) -> None:
        ...
        if len(data) < 20:
            self.logger.warning(
                "I dati hanno meno di 20 osservazioni, il modello potrebbe essere inaffidabile"
            )

        if data.isnull().any().any():
            missing_info = data.isnull().sum()
            self.logger.warning(
                f"I dati contengono valori mancanti:\n{missing_info[missing_info > 0]}"
            )
    ...
    def check_stationarity(self) -> Dict[str, Dict[str, Any]]:
        ...
        if self.training_data is None:
            raise ModelTrainingError(
                "Nessun dato di addestramento disponibile per il controllo di stazionarietà"
            )
        ...
        for col in self.training_data.columns:
            try:
                ...
                results[col] = {
                    ...
                    "recommendation": "stazionaria"
                    if adf_result[1] < 0.05
                    else "non-stazionaria (considera differenziazione)",
                }
