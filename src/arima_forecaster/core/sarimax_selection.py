...
class SARIMAXModelSelector:
    ...
    def search(
        ...
        try:
            ...
            self.logger.info(
                f"Avvio selezione modello {model_type} usando {self.information_criterion.upper()}{exog_info}"
            )
            self.logger.info(
            ...
            if self.n_jobs == 1:
                ...
                self.results = self._fit_models_sequential(
                    series, exog, param_combinations, verbose
                )
            else:
            ...
            if self.results:
                ...
                if not valid_results:
                    self.logger.warning(
                        "Tutti i modelli sembrano avere problemi di overfitting - usando il migliore disponibile"
                    )
                ...
                self.logger.info(
                    f"Miglior modello SARIMAX: {self.best_order}x{self.best_seasonal_order}"
                )
                if exog is not None:
    ...
    def _fit_models_sequential(
        ...
        iterator = (
            tqdm(param_combinations, desc="Test modelli SARIMAX") if verbose else param_combinations
        )

        for order, seasonal_order in iterator:
            try:
                model = SARIMAXForecaster(
                    order=order, seasonal_order=seasonal_order, exog_names=self.exog_names
                )
    ...
    def _fit_models_parallel(
        ...
        with ProcessPoolExecutor(max_workers=self.n_jobs) as executor:
            ...
            future_to_params = {
                executor.submit(
                    _fit_sarimax_model, series, exog, order, seasonal_order, self.exog_names
        ...
        try:
            ...
            if self.exog_names and any(r.get("n_exog", 0) > 0 for r in sorted_results):
                ...
                if exog_analysis is not None:
                    ...
                    var_significance = (
                        exog_analysis.groupby("variable")["significant"]
                        .mean()
                    ...
                    ax2.text(
                        0.5,
                        0.5,
                        "Nessuna analisi variabili esogene disponibile",
                        ha="center",
                        va="center",
                ...
                ax2.text(
                    0.5,
                    0.5,
                    "Nessuna variabile esogena utilizzata",
                    ha="center",
                    va="center",
                    transform=ax2.transAxes,
                )
    ...
    def _filter_suspicious_results(self, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        self.logger.info(
            f"Validazione risultati: AIC mediana={median_aic:.2f}, range ragionevole=[{min_reasonable_aic:.2f}, {max_reasonable_aic:.2f}]"
        )
        ...
        for result in results:
            ...
            if suspicious:
                filtered_count += 1
                self.logger.debug(
                    f"Filtrato modello {order}x{seasonal_order}: {', '.join(reasons)}"
                )
            else:
...
def _fit_sarimax_model(
    ...
    """
    try:
        model = SARIMAXForecaster(order=order, seasonal_order=seasonal_order, exog_names=exog_names)
        model.fit(series, exog=exog, validate_input=False)
