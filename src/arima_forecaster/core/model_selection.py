...
class ARIMAModelSelector:
    ...
    def search(
        self, series: pd.Series, verbose: bool = True, max_models: Optional[int] = None
    ) -> Tuple[int, int, int]:
        """
        ...
        """
        self.logger.info(
            f"Avvio selezione modello ARIMA con criterio {self.information_criterion.upper()}"
        )
        self.results = []
        ...
        self.logger.info(
            f"Miglior modello: ARIMA{best_order} con {self.information_criterion.upper()}={best_criterion:.2f}"
        )
