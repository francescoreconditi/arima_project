...
class GPUARIMAModelSelector(ARIMAModelSelector, GPUModelTrainer):
    ...
    def __init__(
        ...
        logger.info(
            f"GPUARIMAModelSelector inizializzato: "
            f"GPU={self.use_gpu}, max_parallel={self.max_parallel_models}"
        )

    def search_multiple_series(
        self, series_list: List[pd.Series], n_jobs: Optional[int] = None, show_progress: bool = True
    ) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(
            f"Grid Search completato: {len(results)} serie in {total_time:.2f}s "
            f"({total_time / len(series_list):.3f}s per serie)"
        )

        return results

    def _gpu_parallel_search(
        self, series_list: List[pd.Series], show_progress: bool = True
    ) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"GPU Parallel Search: batch_size={batch_size}, total_batches={len(series_list) // batch_size + 1}"
        )
        ...
        if show_progress:
            try:
                ...
                series_iterator = tqdm(
                    range(0, len(series_list), batch_size), desc="GPU ARIMA Search", unit="batch"
                )
            except ImportError:
    ...
    def _prepare_gpu_series(self, series: pd.Series) -> Union[pd.Series, Any]:
        ...
        try:
            ...
            return pd.Series(
                self.gpu_manager.to_cpu(gpu_values), index=series.index, name=series.name
            )
        except:
            # Fallback su CPU
            return series

    def _gpu_single_series_search(
        self, series: pd.Series, param_combinations: List[Tuple]
    ) -> Dict[str, Any]:
    ...
    def _cpu_parallel_search(
        ...
        try:
            ...
            results = Parallel(n_jobs=n_jobs)(
                delayed(self._single_series_wrapper)(series) for series in series_list
            )
...
class GPUSARIMAModelSelector(SARIMAModelSelector, GPUModelTrainer):
    ...
    def __init__(
        ...
        logger.info(
            f"GPUSARIMAModelSelector inizializzato: "
            f"GPU={self.use_gpu}, max_parallel={self.max_parallel_models}"
        )
    ...
    def _gpu_parallel_sarima_search(
        ...
        if show_progress:
            try:
                ...
                series_iterator = tqdm(
                    range(0, len(series_list), batch_size), desc="GPU SARIMA Search", unit="batch"
                )
            except ImportError:
                series_iterator = range(0, len(series_list), batch_size)
        else:
    ...
    def _cpu_parallel_sarima_search(
        ...
        try:
            ...
            results = Parallel(n_jobs=n_jobs)(
                delayed(self._sarima_series_wrapper)(series) for series in series_list
            )
