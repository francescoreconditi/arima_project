...
class ColdStartForecaster:
    ...
    def extract_product_features(self, 
                                ...
                                product_info: Dict[str, Any]) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if len(product_data) > 7:
                ...
                features['cv_demand'] = features['std_demand'] / features['mean_demand'] if features['mean_demand'] > 0 else 0
    ...
    def find_similar_products(self,
                             ...
                             top_n: int = 3) -> List[ProductSimilarity]:
        """
            ...
            for source_product, source_data in products_database.items():
                ...
                if similarity_score >= self.similarity_threshold:
                    similarity = ProductSimilarity(
                        ...
                        features_used=list(set(source_features.keys()) & set(target_features.keys())),
    ...
    def extract_transfer_pattern(self,
                                source_product: str,
                                source_data: pd.Series,
                                source_info: Dict[str, Any]) -> TransferPattern:
        """
        ...
        try:
            ...
            volatility_profile = {
                ...
                'rolling_volatility_30': source_data.rolling(30).std().mean() if len(source_data) >= 30 else source_data.std(),
                'max_volatility': source_data.rolling(7).std().max() if len(source_data) >= 7 else source_data.std()
            }
            ...
            try:
                ...
                model_params['aic'] = arima_model.model_fit_.aic if hasattr(arima_model, 'model_fit_') else None
            except:
    ...
    def pattern_transfer_forecast(self,
                                 transfer_pattern: TransferPattern,
                                 target_product_info: Dict[str, Any],
                                 forecast_days: int = 30,
                                 scaling_factor: float = 1.0) -> Tuple[pd.Series, Dict[str, Any]]:
        """
        ...
        try:
                ...
                if 'weekly' in transfer_pattern.seasonal_pattern:
                    ...
                    for i in range(forecast_days):
                        ...
                        if day_of_week in weekly_pattern:
                            weekly_factor = weekly_pattern[day_of_week] / base_demand if base_demand > 0 else 1
                ...
                if 'monthly' in transfer_pattern.seasonal_pattern:
                    ...
                    for i in range(forecast_days):
                        ...
                        if month in monthly_pattern:
                            monthly_factor = monthly_pattern[month] / base_demand if base_demand > 0 else 1
            ...
            fallback_series = pd.Series(
                ...
                index=pd.date_range(start=pd.Timestamp.now().date(), periods=forecast_days, freq='D')
            )
            return fallback_series, {'method': 'fallback', 'error': str(e)}
    
    def analogical_forecast(self,
                           similar_products: List[ProductSimilarity],
                           products_database: Dict[str, Dict],
                           target_product_info: Dict[str, Any],
                           forecast_days: int = 30) -> Tuple[pd.Series, Dict[str, Any]]:
        """
        ...
        try:
            ...
            for similarity in similar_products:
                ...
                if len(recent_pattern) > 0:
                    ...
                    trend_slope = self._calculate_trend_slope(source_series[-60:] if len(source_series) >= 60 else source_series)
    ...
    def _calculate_analogical_scaling(self,
    ...
    def cold_start_forecast(self,
                           target_product_info: Dict[str, Any],
                           products_database: Dict[str, Dict],
                           forecast_days: int = 30,
                           method: str = 'hybrid') -> Tuple[pd.Series, Dict[str, Any]]:
        """
        ...
        """
        try:
            logger.info(f"Avvio Cold Start forecast per prodotto: {target_product_info.get('codice', 'unknown')}")
    ...
    def _hybrid_forecast(self,
                        similar_products: List[ProductSimilarity],
                        products_database: Dict[str, Dict],
                        target_product_info: Dict[str, Any],
                        forecast_days: int) -> Tuple[pd.Series, Dict[str, Any]]:
        ...
        try:
            ...
            hybrid_forecast = (pattern_forecast * pattern_weight + 
                             analogical_forecast * analogical_weight)
