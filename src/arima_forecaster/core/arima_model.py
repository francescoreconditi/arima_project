...
class ARIMAForecaster:
        ...
        try:
            ...
            if self.use_cache:
                ...
                if cached_model is not None:
                    ...
                    self.logger.info(f"CACHE HIT - Modello caricato da cache (hit_rate: {cache_stats['hit_rate']:.1%})")
            ...
            # Log risultati
            self.logger.info(f"Modello ARIMA addestrato con successo (fit_time: {self.fit_time:.3f}s)")
            self.logger.info(f"AIC: {self.fitted_model.aic:.2f}")
            self.logger.info(f"BIC: {self.fitted_model.bic:.2f}")
    ...
    def _get_smart_fit_params(self, series: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.debug(f"Smart params computed: volatility={volatility_level}, "
        ...
        try:
            ...
            if self.use_vectorized_ops:
                ...
                if len(autocorr) > 12:
                    ...
                    seasonal_lags = [min(lag, len(autocorr)-1) for lag in [4, 7, 12] if lag < len(autocorr)]
                    if seasonal_lags:
                ...
                with ManagedArray(len(series), dtype='float') as temp_buffer:
                    ...
                    if np.any(outlier_mask):
                        ...
                        for idx in outlier_indices:
                            ...
                            window_values = window_values[window_values != clean_values[idx]]  # Escludi il valore outlier
                            
                            if len(window_values) > 0:
                                clean_values[idx] = np.mean(window_values)
                        
                        # Crea serie pulita mantenendo indice originale
                        cleaned_series = pd.Series(clean_values, index=series.index, name=series.name)
    ...
    def _get_smart_fit_params_from_analysis(self, series_analysis: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.debug(f"Smart params from analysis: volatility={volatility_level}, "
                            f"trend={has_trend}, seasonal={has_seasonality}")
