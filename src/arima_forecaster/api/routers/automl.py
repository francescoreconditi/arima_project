...
class ParameterSpace(BaseModel):
    ...
    choices: Optional[List[Union[str, int, float]]] = Field(None, description="Scelte (categorical)")
...
class OptunaOptimizationRequest(BaseModel):
    ...
    optimization_direction: str = Field("minimize", description="Direzione ottimizzazione (minimize/maximize)")
...
class HyperoptSearchResponse(BaseModel):
    ...
    exploration_vs_exploitation: Dict[str, float] = Field(..., description="Bilancio exploration/exploitation")
...
class ScikitOptimizeResponse(BaseModel):
    ...
    expected_improvement: List[float] = Field(..., description="Expected improvement per iterazione")
...
class OptimizationHistoryResponse(BaseModel):
    ...
    parameter_evolution: Dict[str, List[float]] = Field(..., description="Evoluzione parametri nel tempo")
    objective_evolution: List[float] = Field(..., description="Evoluzione obiettivo")
    best_parameters_over_time: List[Dict[str, Any]] = Field(..., description="Migliori parametri per iterazione")
...
class MultiObjectiveRequest(BaseModel):
    ...
    training_data: Dict[str, Any] = Field(..., description="Dati training")
    parameter_space: List[ParameterSpace] = Field(..., description="Spazio parametri")
    objectives: List[str] = Field(..., description="Lista obiettivi (es: ['aic', 'mae', 'complexity'])")
    objective_weights: Optional[Dict[str, float]] = Field(None, description="Pesi obiettivi (se weighted sum)")
...
class MultiObjectiveResponse(BaseModel):
    ...
    convergence_metrics: Dict[str, float] = Field(..., description="Metriche convergenza")
    objective_trade_offs: Dict[str, Dict[str, float]] = Field(..., description="Trade-off tra obiettivi")
    optimization_time: float = Field(..., description="Tempo ottimizzazione")
...
class EnsembleStackingRequest(BaseModel):
    ...
    meta_learner_type: str = Field("linear", description="Tipo meta-learner (linear/ridge/lasso/rf/gbm)")
    ...
    regularization_strength: Optional[float] = Field(None, description="Forza regolarizzazione meta-learner")
...
class EnsembleStackingResponse(BaseModel):
    ...
    cross_validation_scores: Dict[str, List[float]] = Field(..., description="Score CV per modello base")
    ensemble_vs_best_base: Dict[str, float] = Field(..., description="Miglioramento vs miglior base model")
    feature_importance: Optional[Dict[str, float]] = Field(None, description="Importanza feature meta-learner")
...
class EnsembleVotingRequest(BaseModel):
    ...
    model_weights: Optional[Dict[str, float]] = Field(None, description="Pesi modelli (weighted voting)")
    weight_optimization_method: str = Field("performance", description="Metodo calcolo pesi (performance/diversity/combined)")
...
class EnsembleVotingResponse(BaseModel):
    ...
    individual_vs_ensemble: Dict[str, Dict[str, float]] = Field(..., description="Performance individuali vs ensemble")
    weight_optimization_details: Dict[str, Any] = Field(..., description="Dettagli ottimizzazione pesi")
...
class EnsembleBaggingRequest(BaseModel):
    ...
    aggregation_method: str = Field("mean", description="Metodo aggregazione (mean/median/trimmed_mean)")
...
class EnsembleBaggingResponse(BaseModel):
    ...
    individual_model_performance: List[Dict[str, float]] = Field(..., description="Performance modelli individuali")
    variance_reduction: Dict[str, float] = Field(..., description="Riduzione varianza vs modello singolo")
    bias_variance_decomposition: Dict[str, float] = Field(..., description="Decomposizione bias-variance")
    ...
    try:
        ...
        for param in request.parameter_space:
            if param.parameter_type == "categorical" and not param.choices:
                raise HTTPException(status_code=400, detail=f"Parametro categorico {param.parameter_name} deve avere choices")
            elif param.parameter_type in ["int", "float"] and (param.min_value is None or param.max_value is None):
                raise HTTPException(status_code=400, detail=f"Parametro numerico {param.parameter_name} deve avere min/max")
        ...
        for trial_num in range(request.n_trials):
            ...
            for param in request.parameter_space:
                if param.parameter_type == "int":
                    if param.step:
                        ...
                        trial_params[param.parameter_name] = np.random.randint(int(param.min_value), int(param.max_value) + 1)
                        
                elif param.parameter_type == "float":
                    if param.log_scale:
                        trial_params[param.parameter_name] = np.random.lognormal(
                            np.log(param.min_value), 
                            np.log(param.max_value / param.min_value) / 3
                        )
                    else:
                        trial_params[param.parameter_name] = np.random.uniform(param.min_value, param.max_value)
            ...
            if request.pruning_enabled and np.random.random() < 0.15:  # 15% pruned
                trial_state = "PRUNED"
                objective_value = float('inf') if request.optimization_direction == "minimize" else float('-inf')
            ...
            trials_history.append(OptunaTrial(
        ...
        convergence_analysis = {
            ...
            "final_improvement_rate": round(abs(improvement_curve[-5:].mean()), 4) if len(improvement_curve) >= 5 else 0,
    ...
    try:
        ...
        for eval_num in range(request.max_evals):
            ...
            if eval_num < 10:  # Exploration phase
                ...
                recent_improvement = (loss_history[-5:][0] - loss_history[-1]) if len(loss_history) >= 5 else 0
            ...
            if (request.early_stopping_rounds and eval_num >= request.early_stopping_rounds and
                len(loss_history) >= request.early_stopping_rounds):
        ...
        exploitation_ratio = exploitation_evals / len(acquisition_values) if acquisition_values else 0
    ...
    try:
        ...
        for call_num in range(request.n_calls):
            ...
            if call_num == 0:
                ...
                recent_improvement = convergence_curve[-2] - convergence_curve[-1] if len(convergence_curve) >= 2 else 0
                if recent_improvement > 0:
                    ei = acquisition * 0.9
                else:
                    ei = acquisition * 0.6
        ...
        if request.base_estimator == "GP":
            ...
            relative_improvement = improvement / convergence_curve[0] if convergence_curve[0] > 0 else 0
...
async def get_optimization_history(
    ...
    try:
            ...
            if param_name == "d":
                ...
                for i in range(n_iterations):
                    if i < 10:  # Exploration iniziale
                        ...
                        value = optimal_value if np.random.random() < 0.9 else optimal_value + np.random.randint(-1, 2)
        ...
        for i in range(0, n_iterations, max(1, n_iterations // 20)):  # Sample ~20 points
            if objective_evolution[i] < best_obj:
                ...
                best_parameters_over_time.append({
                    "iteration": i + 1,
                    "parameters": {param: parameter_evolution[param][i] for param in param_names},
        ...
        convergence_metrics = {
            ...
            "final_improvement": round((objective_evolution[0] - best_so_far) / objective_evolution[0], 3)
        }
        ...
        for param_name in param_names:
            ...
            if frequency > 0.4:
                optimization_insights.append(f"Parameter '{param_name}' shows strong preference for value {most_common} ({frequency:.0%} of trials)")
        
        # Convergence analysis
        convergence_point = len(improvements) * 2 if improvements else n_iterations
        if convergence_point < n_iterations * 0.8:
            optimization_insights.append(f"Objective converged after ~{convergence_point/n_iterations:.0%} of trials")
        
        # Exploration efficiency
        if exploration_efficiency > 0.7:
            optimization_insights.append("High exploration efficiency - good parameter space coverage")
        elif exploration_efficiency < 0.3:
            optimization_insights.append("Low exploration efficiency - consider increasing n_trials or adjusting search strategy")
        
        # Stability insights
        if stability_score > 0.8:
            optimization_insights.append("High stability in final phase - optimization converged well")
        elif stability_score < 0.5:
            optimization_insights.append("Low stability - may need more trials or different algorithm")
...
async def optimize_multi_objective(
    ...
    try:
        ...
        for _ in range(request.population_size * 2):  # Generate più soluzioni per selezione
            ...
            for param in request.parameter_space:
                if param.parameter_type == "int":
                    solution_params[param.parameter_name] = np.random.randint(int(param.min_value), int(param.max_value) + 1)
                elif param.parameter_type == "float":
                    solution_params[param.parameter_name] = np.random.uniform(param.min_value, param.max_value)
                elif param.parameter_type == "categorical":
            ...
            for obj in request.objectives:
                if obj == "aic":
                    base_aic = 300
                    complexity = sum([v for v in solution_params.values() if isinstance(v, (int, float))])
                    ...
                    complexity = sum([v for v in solution_params.values() if isinstance(v, (int, float))])
                    if complexity <= 3:
                        objective_values[obj] = base_mae - complexity * 0.3 + np.random.normal(0, 0.5)
                    else:
                        objective_values[obj] = base_mae - 3 * 0.3 + (complexity - 3) * 0.2 + np.random.normal(0, 0.5)
        ...
        if len(request.objectives) >= 2:
            for i, obj1 in enumerate(request.objectives):
                for obj2 in request.objectives[i+1:]:
                    ...
                    values1 = [sol.objective_values[obj1] for sol in pareto_solutions if obj1 in sol.objective_values]
                    values2 = [sol.objective_values[obj2] for sol in pareto_solutions if obj2 in sol.objective_values]
...
async def create_ensemble_stacking(
    request: EnsembleStackingRequest,
    services: tuple = Depends(get_automl_services)
):
    """
    ...
    try:
        ...
        if len(request.base_model_ids) < 2:
            raise HTTPException(status_code=400, detail="Servono almeno 2 modelli base per stacking")
        ...
        for model_id in request.base_model_ids:
            ...
            model_type = "arima" if "arima" in model_id else "sarima" if "sarima" in model_id else "var"
        ...
        total_inverse_mae = sum([1/np.mean(scores) for scores in cross_validation_scores.values()])
        base_model_weights = {}
        ...
        weighted_mae = sum([
            base_model_weights[info["model_id"]] * info["base_mae"]
            for info in base_models_info
        ])
        
        # Stacking bonus (diversity benefit)
        stacking_improvement = 0.05 + len(request.base_model_ids) * 0.02  # More models = more improvement
        ...
        meta_learner_features = ["base_pred_" + str(i+1) for i in range(len(request.base_model_ids))]
...
async def create_ensemble_voting(
    ...
    try:
        ...
        if len(request.base_model_ids) < 2:
            raise HTTPException(status_code=400, detail="Servono almeno 2 modelli per voting ensemble")
        ...
        for model_id in request.base_model_ids:
            model_type = "arima" if "arima" in model_id else "sarima" if "sarima" in model_id else "prophet"
        ...
        if request.voting_method == "weighted" and request.model_weights:
            ...
            total_inverse_mae = sum([1/individual_performance[mid]["mae"] for mid in request.base_model_ids])
            initial_weights = {}
        ...
        if request.weight_optimization_method == "performance":
            ...
            for model_id in initial_weights:
                ...
                final_model_weights[model_id] = max(0.05, initial_weights[model_id] + diversity_bonus)
        ...
        best_individual_mae = min([individual_performance[mid]["mae"] for mid in request.base_model_ids])
        ...
        for model_id in request.base_model_ids:
            ...
            individual_vs_ensemble[model_id] = {
                ...
                "improvement_percentage": round((improvement / individual_mae) * 100, 1) if individual_mae > 0 else 0
            }
        
        # Weight optimization details
        weight_optimization_details = {
            "optimization_method": request.weight_optimization_method,
            "initial_weights": {mid: round(initial_weights.get(mid, 0), 3) for mid in request.base_model_ids},
...
async def create_ensemble_bagging(
    request: EnsembleBaggingRequest,
    services: tuple = Depends(get_automl_services)
):
    """
    ...
    try:
        ...
        if not 0.1 <= request.bootstrap_sample_size <= 1.0:
            raise HTTPException(status_code=400, detail="Bootstrap sample size deve essere tra 0.1 e 1.0")
        ...
        for estimator_idx in range(request.n_estimators):
            ...
            bootstrap_sample = np.random.choice(total_sample_indices, size=sample_size, replace=True)
        ...
        average_sample_size = np.mean([perf["sample_size"] for perf in individual_model_performance])
        ...
        diversity_index = np.std(individual_maes) / np.mean(individual_maes) if individual_maes else 0
        ...
        bootstrap_stats = {
            ...
            "unique_samples_per_model": round(np.mean([perf["bootstrap_indices"] for perf in individual_model_performance]), 1)
        }
        ...
        ensemble_mape = np.mean(np.abs((ground_truth - ensemble_predictions) / (ground_truth + 1e-8))) * 100
        ...
        if request.out_of_bag_evaluation:
            ...
            for sample_idx in range(min(50, n_total_samples)):  # Limita per performance
                ...
                if len(models_without_sample) >= 2:  # Serve almeno 2 modelli
                    ...
                    sample_predictions = [all_predictions[i][sample_idx % len(ensemble_predictions)] 
        ...
        variance_reduction_ratio = (individual_variance - ensemble_variance) / individual_variance if individual_variance > 0 else 0
        ...
        variance_reduction = {
            ...
            "theoretical_reduction": round(1 - 1/request.n_estimators, 3)  # 1/n for independent models
        }
