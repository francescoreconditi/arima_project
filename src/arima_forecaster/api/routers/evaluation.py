...
class ResidualAnalysisResponse(BaseModel):
    ...
    residuals_statistics: Dict[str, float] = Field(
        ..., description="Statistiche descrittive residui"
    )
    normality_tests: Dict[str, Dict[str, float]] = Field(..., description="Test normalità residui")
    autocorrelation_tests: Dict[str, Dict[str, float]] = Field(
        ..., description="Test autocorrelazione"
    )
    heteroscedasticity_tests: Dict[str, Dict[str, float]] = Field(
        ..., description="Test eteroschedasticità"
    )
...
class ModelDiagnosticsResponse(BaseModel):
    ...
    statistical_tests: Dict[str, Dict[str, Any]] = Field(
        ..., description="Risultati test statistici"
    )
...
class CrossValidationRequest(BaseModel):
    ...
    metrics: List[str] = Field(
        default=["mae", "rmse", "mape", "smape", "mase"], description="Metriche da calcolare"
    )
...
class CrossValidationResponse(BaseModel):
    ...
    aggregate_metrics: Dict[str, Dict[str, float]] = Field(
        ..., description="Metriche aggregate (mean, std)"
    )
...
class WalkForwardRequest(BaseModel):
    ...
    metrics: List[str] = Field(default=["mae", "rmse", "mape"], description="Metriche da calcolare")
...
class WalkForwardResponse(BaseModel):
    ...
    performance_trend: Dict[str, List[float]] = Field(
        ..., description="Trend performance nel tempo"
    )
    model_stability: Dict[str, float] = Field(..., description="Metriche stabilità modello")
    degradation_analysis: Dict[str, Any] = Field(
        ..., description="Analisi degradazione performance"
    )
...
class ForecastIntervalsResponse(BaseModel):
    ...
    forecast_intervals: Dict[str, Dict[str, List[float]]] = Field(
        ..., description="Intervalli per livello confidenza"
    )
    ...
    reliability_assessment: Dict[str, float] = Field(
        ..., description="Assessment affidabilità intervalli"
    )
...
async def analyze_residuals(model_id: str, services: tuple = Depends(get_evaluation_services)):
    """
    ...
    try:
        ...
        residuals_statistics = {
            ...
            "skewness": round(
                float(
                    np.nan_to_num(
                        len(residuals_array)
                        * np.sum((residuals_array - np.mean(residuals_array)) ** 3)
                        / (
                            (len(residuals_array) - 1)
                            * (len(residuals_array) - 2)
                            * np.std(residuals_array) ** 3
                        )
                    )
                ),
                3,
            ...
            "kurtosis": round(
                float(
                    np.nan_to_num(
                        np.sum((residuals_array - np.mean(residuals_array)) ** 4)
                        / (len(residuals_array) * np.std(residuals_array) ** 4)
                    )
                ),
                3,
            ),
        ...
        outlier_indices_iqr = np.where(
            (residuals_array < lower_bound) | (residuals_array > upper_bound)
        )[0].tolist()
        ...
        outlier_detection = {
            ...
            "outlier_percentage": round(
                len(set(outlier_indices_z + outlier_indices_iqr)) / len(residuals_array) * 100, 2
            ),
        }
        ...
        if normality_tests["jarque_bera"]["is_normal"]:
            diagnostic_summary.append(
                f"Residuals appear normally distributed (JB p-value: {normality_tests['jarque_bera']['p_value']})"
            )
        else:
            diagnostic_summary.append(
                f"Residuals show non-normal distribution (JB p-value: {normality_tests['jarque_bera']['p_value']})"
            )
        ...
        if autocorrelation_tests["ljung_box"]["no_autocorr"]:
            ...
            diagnostic_summary.append(
                "Significant autocorrelation present - model may be misspecified"
            )
        ...
        scores.append(
            1.0 - min(0.5, outlier_detection["outlier_percentage"] / 10)
        )  # Penalizza outliers
...
async def perform_ljung_box_test(request: LjungBoxTestRequest):
    ...
    try:
        if len(request.residuals) < request.lags + 5:
            raise HTTPException(
                status_code=400, detail="Servono almeno 'lags + 5' osservazioni per il test"
            )
        ...
        for lag in range(1, request.lags + 1):
            if n - lag > 0:
                ...
                numerator = np.sum(
                    (residuals_array[:-lag] - mean_resid) * (residuals_array[lag:] - mean_resid)
                )
        ...
        if is_significant:
            ...
            interpretation = (
                f"No evidence of autocorrelation - model appears adequate (p={p_value:.3f})"
            )
...
async def perform_jarque_bera_test(request: JarqueBeraTestRequest):
    ...
    try:
        if len(request.residuals) < 8:
            raise HTTPException(
                status_code=400, detail="Servono almeno 8 osservazioni per test affidabile"
            )
        ...
        # Skewness (asimmetria)
        skewness = (
            np.sum((residuals_array - mean_resid) ** 3) / (n * std_resid**3)
            if std_resid > 0
            else 0.0
        )
        ...
        # Kurtosis (curtosi)
        kurtosis = (
            np.sum((residuals_array - mean_resid) ** 4) / (n * std_resid**4)
            if std_resid > 0
            else 3.0
        )
...
async def perform_arch_test(request: ARCHTestRequest):
    ...
    try:
        if len(request.residuals) < request.lags + 10:
            raise HTTPException(
                status_code=400, detail="Servono più osservazioni per test ARCH affidabile"
            )
        ...
        if is_significant:
            interpretation = (
                f"Evidence of ARCH effects detected (p={p_value:.3f}) - variance is not constant"
            )
        else:
            interpretation = (
                f"No evidence of ARCH effects - variance appears constant (p={p_value:.3f})"
            )
...
async def get_model_diagnostics(model_id: str, services: tuple = Depends(get_evaluation_services)):
    """
    ...
    try:
        ...
        if len(warning_flags) == 0:
            ...
            if not lb_passes:
                recommendations.append(
                    "Consider increasing ARIMA/SARIMA order to address autocorrelation"
                )
            if not jb_passes:
                recommendations.append(
                    "Consider transformation (log, Box-Cox) to improve normality"
                )
            if not arch_passes:
        ...
        # Test statistici (40%)
        tests_passed = sum(
            [test["passed"] for test in statistical_tests.values() if "passed" in test]
        )
        ...
        stability_score = (
            0.8 if len(warning_flags) == 0 else max(0.3, 0.8 - len(warning_flags) * 0.1)
        )
...
async def perform_cross_validation(
    request: CrossValidationRequest, services: tuple = Depends(get_evaluation_services)
):
    """
    ...
    try:
        ...
        for fold in range(request.cv_folds):
            ...
            if "smape" in request.metrics:
                smape = 100 * np.mean(
                    2
                    * np.abs(predictions - test_values)
                    / (np.abs(predictions) + np.abs(test_values))
                )
        ...
        if not validation_results:
            raise HTTPException(
                status_code=400, detail="Impossibile completare CV con parametri forniti"
            )
        ...
        for metric in request.metrics:
            metric_values = [
                fold.metrics.get(metric, 0) for fold in validation_results if metric in fold.metrics
            ]
            if metric_values:
        ...
        stability_metrics = {
            f"cv_coefficient_{primary_metric}": round(
                np.std(primary_values) / np.mean(primary_values), 4
            )
            if np.mean(primary_values) > 0
            else 0,
        ...
        if cv_coeff < 0.1:
            cv_recommendations.append(
                f"Model shows very stable performance across folds (CV={cv_coeff:.1%})"
            )
        elif cv_coeff < 0.2:
            cv_recommendations.append(
                f"Model shows stable performance across folds (CV={cv_coeff:.1%})"
            )
        else:
            cv_recommendations.append(
                f"Model shows variable performance across folds (CV={cv_coeff:.1%}) - investigate instability"
            )
        ...
        if trend == "stable":
            ...
            cv_recommendations.append(
                "Performance appears to degrade over time - monitor for concept drift"
            )
        elif trend == "improving":
            cv_recommendations.append(
                "Performance improves over time - model may benefit from more recent data"
            )
        ...
        if len(validation_results) > 1:
            ...
            cv_recommendations.append(
                f"Fold {best_fold.fold_number} shows best performance - investigate data characteristics"
            )
        ...
        if total_test_obs < 50:
            cv_recommendations.append(
                "Small test sample size - consider more data for reliable validation"
            )
...
async def perform_walk_forward_validation(
    request: WalkForwardRequest, services: tuple = Depends(get_evaluation_services)
):
    """
    ...
    try:
        ...
        if not validation_steps:
            raise HTTPException(
                status_code=400, detail="Impossibile completare walk-forward con parametri forniti"
            )
        ...
        for metric in request.metrics:
            metric_values = [
                step.metrics.get(metric, 0) for step in validation_steps if metric in step.metrics
            ]
            if metric_values:
        ...
        if refit_steps and non_refit_steps:
            avg_before_refit = np.mean(
                [step.metrics.get(primary_metric, 0) for step in non_refit_steps]
            )
            avg_after_refit = np.mean([step.metrics.get(primary_metric, 0) for step in refit_steps])
            refit_improvement = (
                (avg_before_refit - avg_after_refit) / avg_before_refit
                if avg_before_refit > 0
                else 0
            )
        else:
            refit_improvement = 0
        ...
        model_stability = {
            ...
            "performance_volatility": round(
                np.std([step.metrics.get(primary_metric, 0) for step in validation_steps]), 3
            ),
        ...
        if refit_improvement > 0.05:
            refit_recommendations.append(
                f"Model refitting shows significant improvement ({refit_improvement:.2%}) - consider more frequent refits"
            )
        elif refit_improvement < -0.02:
            refit_recommendations.append(
                f"Model refitting appears counterproductive - consider less frequent refits"
            )
        else:
            refit_recommendations.append(
                f"Current refit frequency ({request.refit_frequency}) appears adequate"
            )

        if decay_rate > 0.01:
            refit_recommendations.append(
                "Significant performance decay detected - increase refit frequency"
            )
        ...
        if optimal_refit != request.refit_frequency:
            refit_recommendations.append(
                f"Consider adjusting refit frequency to {optimal_refit} steps"
            )
        ...
        if cv < 0.1:
            ...
            refit_recommendations.append(
                "High performance variability - investigate data stability"
            )
    ...
    try:
        ...
        for level in confidence_levels:
            if level <= 0 or level >= 1:
                raise HTTPException(
                    status_code=400, detail="Livelli confidenza devono essere tra 0 e 1"
                )
        ...
        for conf_level in confidence_levels:
            ...
            forecast_intervals[conf_key] = {"lower": lower_bounds, "upper": upper_bounds}
