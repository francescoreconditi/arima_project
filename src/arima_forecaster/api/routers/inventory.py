...
class MovementClassificationResponse(BaseModel):
    ...
    summary_by_class: Dict[str, Dict[str, Union[int, float]]] = Field(..., description="Riassunto per classe")
...
class SlowFastOptimizationRequest(BaseModel):
    ...
    classification_results: List[ProductClassification] = Field(..., description="Risultati classificazione ABC/XYZ")
    ...
    service_level_targets: Optional[Dict[str, float]] = Field(
        default={"A": 0.98, "B": 0.95, "C": 0.90},
        description="Service level target per classe ABC"
    )
...
class SafetyStockRequest(BaseModel):
    ...
    seasonal_adjustment: Optional[bool] = Field(True, description="Applica aggiustamento stagionale")
...
class LocationData(BaseModel):
    ...
    transportation_costs: Dict[str, float] = Field(..., description="Costi trasporto verso altre location")
...
class MultiEchelonRequest(BaseModel):
    ...
    centralization_options: Optional[Dict[str, Any]] = Field(None, description="Opzioni centralizzazione")
...
class MultiEchelonResponse(BaseModel):
    ...
    recommended_allocation: Dict[str, Dict[str, float]] = Field(..., description="Allocazione per location/prodotto")
...
class CapacityConstraintsRequest(BaseModel):
    ...
    products: List[Dict[str, Any]] = Field(..., description="Prodotti con dati dimensioni/peso/valore")
    total_capacity: Dict[str, float] = Field(..., description="Capacità totali {volume, weight, value}")
...
class CapacityConstraintsResponse(BaseModel):
    ...
    optimization_id: str
    optimized_portfolio: List[Dict[str, Any]] = Field(..., description="Portfolio prodotti ottimizzato")
...
class KittingAnalysisRequest(BaseModel):
    ...
    demand_correlation: Optional[Dict[str, float]] = Field(None, description="Correlazione domanda tra prodotti")
    assembly_costs: Dict[str, float] = Field(..., description="Costi assemblaggio per tipo bundle")
    storage_cost_difference: float = Field(..., description="Differenza costo storage bundle vs componenti")
...
class KittingAnalysisResponse(BaseModel):
    ...
    make_to_stock_vs_assemble: Dict[str, str] = Field(..., description="Raccomandazione per prodotto")
...
async def classify_product_movement(
    ...
    try:
        ...
        for data in products_data:
            ...
            classified_products.append(ProductClassification(
        ...
        if a_products > 0:
            recommendations.append("Focus on demand sensing for classe A products - High impact on revenue")
        if high_variability > 0:
            recommendations.append(f"{high_variability} products with high variability - Implement dynamic safety stock")
        ...
        for class_key in summary_by_class:
            ...
            if abc == "A" and xyz == "X":
                recommendations.append(f"Classe {class_key} ({count} products): High-value predictable - EOQ optimization, low safety stock")
            elif abc == "A" and xyz == "Z":
                recommendations.append(f"Classe {class_key} ({count} products): High-value volatile - High safety stock, frequent review")
            elif abc == "C" and xyz == "Z":
                recommendations.append(f"Classe {class_key} ({count} products): Low-value volatile - Basic rules, accept stockouts")
...
async def optimize_slow_fast_moving(
    request: SlowFastOptimizationRequest,
    services: Optional[Dict] = Depends(get_inventory_services)
):
    """
    ...
    try:
        ...
        inventory_reduction_pct = (total_investment_reduction / total_annual_revenue) * 100 if total_annual_revenue > 0 else 0
        avg_service_improvement = np.mean([float(r["service_level_improvement"].replace("+", "").replace("%", "")) for r in recommendations])
        estimated_cost_savings = total_investment_reduction * request.inventory_costs["holding_cost_rate"]
        ...
        summary_metrics = {
            ...
            "investment_increase_products": len([r for r in recommendations if r["investment_change"] > 0]),
            "investment_decrease_products": len([r for r in recommendations if r["investment_change"] < 0])
        }
...
async def calculate_economic_order_quantity(request: EOQRequest):
    ...
    try:
        ...
        return EOQResponse(
            ...
            annual_ordering_cost=round(final_ordering_cost if 'final_ordering_cost' in locals() else annual_ordering_cost, 2),
            annual_holding_cost=round(final_holding_cost if 'final_holding_cost' in locals() else annual_holding_cost, 2),
    ...
    try:
        ...
        for location in request.locations:
            location_stats[location.location_id] = {
                ...
                "cv": np.std(location.demand_forecast) / np.mean(location.demand_forecast) if np.mean(location.demand_forecast) > 0 else 0
            }
            
            for product in request.products:
                if product not in total_demand:
                    total_demand[product] = {
                        "total": 0,
                        "by_location": {},
                        "pooled_std": 0
                    }
        ...
        if request.risk_pooling_enabled:
            ...
            risk_pooling_benefits = {
                ...
                "inventory_reduction_pct": round((total_safety_stock_reduction / 
                    sum([sum(location.demand_forecast) for location in request.locations])) * 100, 1)
            }
        ...
        total_cost_reduction = (
            risk_pooling_benefits.get("safety_stock_reduction", 0) + 
            transport_savings
        )
        ...
        if request.risk_pooling_enabled and risk_pooling_benefits.get("inventory_reduction_pct", 0) > 15:
            recommendations.append(f"Implement risk pooling - {risk_pooling_benefits['inventory_reduction_pct']}% inventory reduction")
        ...
        if high_transport_locations:
            recommendations.append(f"Consider centralization for {len(high_transport_locations)} locations with low transport costs")
        ...
        if close_pairs:
            recommendations.append(f"Setup transshipment between {len(close_pairs)} location pairs with low transport costs")
...
async def optimize_with_capacity_constraints(
    request: CapacityConstraintsRequest,
    services: Optional[Dict] = Depends(get_inventory_services)
):
    """
    ...
    try:
        ...
        for product in request.products:
            ...
            volume_efficiency = annual_revenue / product.get("volume", 1) if product.get("volume", 0) > 0 else 0
            weight_efficiency = annual_revenue / product.get("weight", 1) if product.get("weight", 0) > 0 else 0
            value_efficiency = annual_revenue / product.get("value", 1) if product.get("value", 0) > 0 else 0
            ...
            if request.constraints_type == "volume":
                ...
                max_vol_eff = max([p.get("annual_demand", 0) * (p.get("value", 0) - p.get("cost", 0)) / p.get("volume", 1) for p in request.products])
                max_weight_eff = max([p.get("annual_demand", 0) * (p.get("value", 0) - p.get("cost", 0)) / p.get("weight", 1) for p in request.products])
                max_val_eff = max([p.get("annual_demand", 0) * (p.get("value", 0) - p.get("cost", 0)) / p.get("value", 1) for p in request.products])
        ...
        for item in products_with_scores:
            ...
            max_qty_volume = remaining_capacity["volume"] / product.get("volume", float('inf')) if product.get("volume", 0) > 0 else float('inf')
            max_qty_weight = remaining_capacity["weight"] / product.get("weight", float('inf')) if product.get("weight", 0) > 0 else float('inf')
            max_qty_value = remaining_capacity["value"] / product.get("value", float('inf')) if product.get("value", 0) > 0 else float('inf')
            ...
            if max_feasible_qty >= 1:  # Almeno 1 unità
                ...
                selected_products.append({
                    ...
                    "annual_revenue": round(item["annual_revenue"] * (max_feasible_qty / demand), 0),
        ...
        capacity_utilization = {
            "volume": round(((request.total_capacity["volume"] - remaining_capacity["volume"]) / request.total_capacity["volume"]) * 100, 1),
            "weight": round(((request.total_capacity["weight"] - remaining_capacity["weight"]) / request.total_capacity["weight"]) * 100, 1),
            "value": round(((request.total_capacity["value"] - remaining_capacity["value"]) / request.total_capacity["value"]) * 100, 1)
        }
...
async def analyze_kitting_strategies(
    request: KittingAnalysisRequest,
    services: Optional[Dict] = Depends(get_inventory_services)
):
    """
    ...
    try:
        ...
        for main_product in request.main_products:
            ...
            for i, comp1 in enumerate(components):
                for j, comp2 in enumerate(components[i+1:], i+1):
                    ...
                    correlation = request.demand_correlation.get(corr_key, 
                                request.demand_correlation.get(reverse_key, 0.5))
            ...
            labor_cost_per_unit = assembly_time * request.assembly_costs.get("labor_per_minute", 0.5)
        ...
        payback_months = abs(total_inventory_cost_change) / (total_assembly_savings / 12) if total_assembly_savings > 0 else float('inf')
        ...
        service_level_impact = {
            ...
            "projected": min(0.98, 0.92 + (avg_correlation * 0.06))  # Miglioramento per correlazione
        }
        ...
        high_roi_bundles = [b for b in recommended_bundles if b["expected_margin_improvement"] > 10 and b["confidence"] > 0.6]
        if high_roi_bundles:
            best_bundle = max(high_roi_bundles, key=lambda x: x["expected_margin_improvement"])
            implementation_priority.append(f"Phase 1: Implement {best_bundle['bundle_id']} - High ROI ({best_bundle['expected_margin_improvement']:.1f}%), low risk")
        
        medium_bundles = [b for b in recommended_bundles if b["strategy"] == "Hybrid (Partial Pre-Assembly)"]
        if medium_bundles:
            implementation_priority.append("Phase 2: Test hybrid approach for medium-confidence bundles")
