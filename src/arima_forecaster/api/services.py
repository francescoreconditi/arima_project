...
class ModelManager:
    ...
    def _save_registry(self):
        ...
        try:
            ...
            for model_id, info in self.model_registry.items():
                ...
                if 'created_at' in registry_copy[model_id] and isinstance(registry_copy[model_id]['created_at'], datetime):
    ...
    async def train_model(
        ...
        """
        try:
            self.logger.info(f"Avvio addestramento modello {request.model_type.upper()} con ID {model_id}")
            ...
            if request.auto_select:
                ...
                if request.model_type == "arima":
                    ...
                    if exog_df is None:
                        raise ModelTrainingError("SARIMAX richiede dati esogeni per l'addestramento")
                    order = (request.order.p, request.order.d, request.order.q)
                    seasonal_order = (
                        request.seasonal_order.P,
                        request.seasonal_order.D,
    ...
    async def train_var_model(
        ...
        try:
            ...
            self.logger.info(f"Serie multivariate: {data.shape[1]} variabili, {len(data)} osservazioni")
            ...
            self.model_registry[model_id].update({
                ...
                "metrics": metrics,
                "model_path": str(model_path),
                "selected_lag": model_info.get("lag_order", 0)  # Lag effettivamente selezionato
            })
            self._save_registry()
...
class ForecastService:
    ...
    async def generate_var_forecast(
        ...
        try:
            ...
            self.logger.info(f"Previsione VAR generata per {len(variables_list)} variabili: {variables_list}")
            return result
    ...
    async def auto_select_model(
        ...
        try:
            ...
            self.logger.info(f"Parametri: max_models={max_models}, criterio={information_criterion}")
            ...
            self.logger.info(f"Miglior modello {model_id} con {information_criterion}={result['best_score']}")
    ...
    async def generate_diagnostics(
        ...
        **Funzioni di Correlazione:**
        ...
        try:
            ...
            if include_residuals and hasattr(model, 'fitted_model') and hasattr(model.fitted_model, 'resid'):
                ...
                diagnostics.residual_stats = {
                    ...
                    "kurtosis": float(residuals.kurtosis()) if hasattr(residuals, 'kurtosis') else 0.0,
                ...
                try:
                    ...
                    try:
                        ...
                        lb_result = acorr_ljungbox(clean_residuals, lags=min(10, len(clean_residuals)//4))
                        ...
                        self.logger.warning("statsmodels.stats.diagnostic non disponibile per test Ljung-Box")
        ...
        **Technical Appendix:**
        ...
        try:
            ...
            if output_filename is None:
                ...
                output_filename = f"report_{model_info.get('model_type', 'model')}_{model_short_id}_{timestamp}"
            ...
            self.logger.info(f"Avvio generazione report {format_type.upper()} per modello {model_id}")
