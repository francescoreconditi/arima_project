...
def main():
    ...
    print(f"\nSplit dataset:")
    print(f"  Training: {len(train_data)} mesi ({train_data.index.min().strftime('%Y-%m')} - {train_data.index.max().strftime('%Y-%m')})")
    print(f"  Test: {len(test_data)} mesi ({test_data.index.min().strftime('%Y-%m')} - {test_data.index.max().strftime('%Y-%m')})")
    ...
    if isinstance(future_forecast, dict):
        ...
        if 'confidence_intervals' in future_forecast:
            ...
            plt.fill_between(future_dates, lower, upper, alpha=0.3, color='red', label='Intervallo Confidenza')
    else:
        future_values = future_forecast
    
    plt.plot(future_dates, future_values, label='Forecast 12 Mesi', color='red', linewidth=2, marker='o')
