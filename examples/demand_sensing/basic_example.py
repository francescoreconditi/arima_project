...
def main():
    ...
    for _, row in contributions.iterrows():
        print(f"   {row['source'].capitalize():<12}: "
    ...
    ax1.plot(days, sensing_result.original_forecast.values, 'b-', 
            label='SARIMA Base', linewidth=2)
    ax1.plot(days, sensing_result.adjusted_forecast.values, 'r--', 
            label='Con Demand Sensing', linewidth=2)
    ax1.fill_between(days, sensing_result.original_forecast.values,
                    sensing_result.adjusted_forecast.values,
                    alpha=0.3, color='green', label='Aggiustamento')
    ...
    if sensing_result.factors_applied:
        ...
        names = [f.factor.name[:15] + '...' if len(f.factor.name) > 15 
                else f.factor.name for f in top_factors]
    ...
    np.random.seed(123)
    simulated_actuals = (
        sensing_result.adjusted_forecast * 
        np.random.uniform(0.9, 1.1, len(sensing_result.adjusted_forecast))
    )
    
    # Applica apprendimento
    sensor.learn_from_actuals(
        sensing_result.adjusted_forecast,
        simulated_actuals
    )
