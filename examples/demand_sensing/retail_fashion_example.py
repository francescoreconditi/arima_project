...
def main():
    ...
    print(f"   📦 Safety stock consigliato: {safety_stock_adjusted:.0f} pezzi "
          f"({inventory_diff:+.0f})")
    ...
    ax1.plot(range(-60, 0), recent_history['sales'].values, 
            'b-', label='Storico', linewidth=2)
    
    # Previsioni
    forecast_days = range(0, len(base_forecast))
    ax1.plot(forecast_days, sensing_result.original_forecast.values, 
            'g-', label='SARIMA', linewidth=2)
    ax1.plot(forecast_days, sensing_result.adjusted_forecast.values, 
    ...
    im = ax2.imshow(daily_impacts, cmap='RdYlGn', aspect='auto', 
                   vmin=-20, vmax=20)
    ...
    ax4 = axes[1, 0]
    cumulative_impact = (sensing_result.adjusted_forecast - 
                        sensing_result.original_forecast).cumsum() * avg_price
    ax4.plot(forecast_days, cumulative_impact.values, 'purple', linewidth=3)
    ax4.fill_between(forecast_days, 0, cumulative_impact.values, 
    ...
    if sensing_result.factors_applied:
        confidences = [f.factor.confidence for f in sensing_result.factors_applied[:len(forecast_days)]]
        ax6.plot(forecast_days[:len(confidences)], confidences, 'orange', 
