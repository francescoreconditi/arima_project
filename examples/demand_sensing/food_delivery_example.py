...
def generate_food_delivery_data(days=200):
    ...
    weather_effect = np.random.choice(
        [1.0, 1.2, 1.0, 0.9, 1.3], days, p=[0.4, 0.2, 0.2, 0.1, 0.1]
    )  # 30% giorni con boost meteo
    ...
    # Combina tutto
    orders = (base_trend * weekly_pattern * meal_variation * weather_effect * special_days) + noise
...
def main():
    ...
    print(
        f"   Weekend factor: {orders_data[orders_data['date'].dt.weekday >= 5]['orders'].mean() / orders_data[orders_data['date'].dt.weekday < 5]['orders'].mean():.1f}x"
    )
    ...
    sensor = EnsembleDemandSensor(
        base_model=model, product_category="food", location=location, config=config
    )
    ...
    if weather_factors:
        ...
        if avg_weather > 0.1:
            food_recs.append("[RAIN] Meteo favorevole per delivery - aumentare staff e scorte")
        elif avg_weather < -0.05:
            food_recs.append("[SUN] Bel tempo previsto - focus su promozioni takeaway")
    ...
    if calendar_factors and np.mean([f.impact for f in calendar_factors]) > 0.1:
        food_recs.append("[CALENDAR] Eventi importanti - preparare menu speciali e extra staff")
    ...
    if social_factors:
        ...
        if avg_social < -0.05:
            food_recs.append("[SOCIAL] Sentiment negativo rilevato - intensificare customer care")
        elif avg_social > 0.1:
            food_recs.append("[SOCIAL] Buzz positivo - capitalizzare con campagne social")
    ...
    ax1.plot(range(-30, 0), recent_data["orders"].values, "b-", label="Storico", linewidth=2)
    ...
    bars = ax2.bar(
        days_of_week,
        weekly_avg,
        color=["lightcoral" if x in ["Sab", "Dom"] else "skyblue" for x in days_of_week],
    )
    ...
    ax3 = axes[0, 2]
    colors = ["green" if x > 0 else "red" if x < -5 else "orange" for x in adjustment_pct.values]
    bars = ax3.bar(forecast_range, adjustment_pct.values, color=colors, alpha=0.7)
    ...
    ax4.plot(
        forecast_range, revenue_cumulative.values, "purple", linewidth=3, marker="o", markersize=4
    )
    ax4.fill_between(forecast_range, 0, revenue_cumulative.values, alpha=0.3, color="purple")
    ...
    ax6.plot(forecast_range, adjusted_staff, "r--", label="Staff Aggiustato", linewidth=2)
    ax6.fill_between(
        forecast_range, base_staff, adjusted_staff, alpha=0.3, color="orange", label="Extra Staff"
    )
    ...
    daily_plan = pd.DataFrame(
        {
            ...
            "Giorno": pd.date_range(
                start=datetime.now(), periods=forecast_horizon, freq="D"
            ).strftime("%A"),
            ...
            "Rider_Necessari": np.ceil(
                sensing_result.adjusted_forecast.values / orders_per_staff
            ).astype(int),
            "Ricavi_Stimati": (sensing_result.adjusted_forecast.values * avg_order_value).astype(
                int
            ),
            "Alert_Level": [
                "[FIRE] ALTO" if x > 20 else "[WARN] MEDIO" if x > 10 else "[OK] NORMALE"
