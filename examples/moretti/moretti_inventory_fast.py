    ...
    def __init__(self):
        self.prodotti = [
            ProdottoMedicale(
                ...
                categoria=ProdottoCategoria.CARROZZINE,
                prezzo_medio=280.0,
                lead_time_giorni=15,
                scorta_minima=20,
                scorta_sicurezza=10,
            ...
            ProdottoMedicale(
                ...
                categoria=ProdottoCategoria.ANTIDECUBITO,
                prezzo_medio=450.0,
                lead_time_giorni=10,
                scorta_minima=15,
                scorta_sicurezza=8,
            ...
            ProdottoMedicale(
                codice="ELT001",
                nome="Saturimetro",
                categoria=ProdottoCategoria.ELETTROMEDICALI,
                prezzo_medio=120.0,
                lead_time_giorni=7,
                scorta_minima=25,
                scorta_sicurezza=12,
        ...
        self.fornitori = {
            "CRZ001": [
                Fornitore(
                    codice="F001",
                    nome="MedSupply Italia",
                    prezzo_base=300.0,
                    sconto_quantita=0.15,
                    lead_time=15,
                    affidabilita=0.95,
                ),
                Fornitore(
            ...
            "MAT001": [
                Fornitore(
                    codice="F003",
                    nome="AntiDecubito Pro",
                    prezzo_base=480.0,
                    sconto_quantita=0.18,
                    lead_time=10,
                    affidabilita=0.98,
                ),
                Fornitore(
                    codice="F004",
                    nome="Medical Comfort",
                    prezzo_base=470.0,
                    sconto_quantita=0.15,
                    lead_time=14,
                    affidabilita=0.90,
...
class SistemaScorteVeloce:
    ...
    def genera_previsioni_future(self, prodotto_codice: str, giorni: int = 30) -> pd.DataFrame:
        ...
        future_dates = pd.date_range(
            start=datetime.now() + timedelta(days=1), periods=giorni, freq="D"
        )

        return pd.DataFrame({"Data": future_dates, "Previsione": forecast})
...
def demo_sistema_veloce():
    ...
    for prodotto in generatore.prodotti:
        ...
        scorte_attuali = np.random.randint(
            int(scorte["punto_riordino"] * 0.5), int(scorte["punto_riordino"] * 1.5)
        )
        ...
        if scorte_attuali <= scorte["punto_riordino"]:
            ...
            giorni_copertura = (scorte_attuali - scorte["punto_riordino"]) / scorte[
                "domanda_giornaliera"
            ]
    ...
    print(
        f"   - EUR {investimento_totale:,.2f} investimento richiesto"
        if riordini_necessari
        else "   - Nessun investimento necessario"
    )
