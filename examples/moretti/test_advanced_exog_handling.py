...
def generate_moretti_exog_data(n_days: int = 365) -> tuple[pd.Series, pd.DataFrame]:
    ...
    tasso_disoccupazione = (
        8 + 2 * np.sin(2 * np.pi * np.arange(n_days) / 365) + np.random.normal(0, 0.3, n_days)
    )
    ...
    spesa_sanitaria = (
        1000 + 200 * np.sin(2 * np.pi * np.arange(n_days) / 90) + np.random.normal(0, 50, n_days)
    )  # Cicli trimestrali
    ...
    search_trends = (
        100 + 30 * np.sin(2 * np.pi * np.arange(n_days) / 365) + np.random.normal(0, 10, n_days)
    )
    ...
    econ_effect = -0.001 * (pil_regionale - 50000) + 0.2 * (
        tasso_disoccupazione - 8
    )  # PIL alto e disoc. bassa = meno domanda
...
def demo_advanced_exog_preprocessing():
    ...
    for method in methods:
        ...
        logger.info(
            f"  - Trasformazioni applicate: {len(report.get('transformations_applied', []))}"
        )
        logger.info(
            f"  - Feature multicollineari rimosse: {len(report.get('multicollinear_features', []))}"
        )
...
def demo_feature_selection_methods():
    ...
    for method in methods:
        ...
        try:
            ...
            forecast_result = selector.forecast_with_exog(
                steps=len(demand_test), exog=exog_test, confidence_intervals=False
            )
    ...
    for method, results in results_comparison.items():
        if "error" not in results:
            logger.info(
                f"{method:<15} {results['selected_features']:<10} "
...
def demo_exog_diagnostics():
    ...
    diagnostic_results = diagnostics.full_diagnostic_suite(
        target_series=demand, exog_data=exog, fitted_model=selector.fitted_model
    )
    ...
    stationary_vars = [
        var for var, result in stationarity.items() if result.get("consensus") == "stationary"
    ]
    ...
    causal_vars = [
        (var, result.get("strength", "unknown"))
    ...
    report_path = diagnostics.save_diagnostic_report(
        output_path=output_dir / "moretti_exog_diagnostic_report.json", format_type="json"
    )
...
def demo_end_to_end_workflow():
    ...
    high_corr = [
        var for var, corr in relationships.get("correlations", {}).items() if abs(corr) > 0.3
    ]
    ...
    diagnostic_results = diagnostics.full_diagnostic_suite(
        target_series=demand_train, exog_data=exog_train, fitted_model=selector.fitted_model
    )

    # 7. Forecasting
    logger.info("\n7. FORECASTING")
    forecast_result = selector.forecast_with_exog(
        steps=len(demand_test), exog=exog_test, confidence_intervals=True
    )
    ...
    logger.info(
        f"  - Original features: {feature_analysis['selection_summary']['original_features']}"
    )
    logger.info(
        f"  - Engineered features: {feature_analysis['selection_summary']['engineered_features']}"
    )
    logger.info(f"  - Final selected: {feature_analysis['selection_summary']['selected_features']}")
    logger.info(
        f"  - Selection ratio: {feature_analysis['selection_summary']['selection_ratio']:.2%}"
    )
    ...
    results_df = pd.DataFrame(
        {
            ...
            "abs_error_pct": abs(
                (demand_test.values - forecast_values.values) / demand_test.values * 100
            ),
        }
    )
...
def main():
    ...
    logger.info("=" * 80)
    logger.info(
        "Testing enterprise-grade exogenous variable handling for medical device forecasting"
    )
    logger.info("=" * 80)
    ...
    try:
        ...
        logger.info(
            f"✅ Diagnostic assessment: {final_results['diagnostic_summary'].get('overall_assessment', 'completed')}"
        )

        logger.info(f"\n📊 Key Performance Indicators:")
        logger.info(
            f"   - Feature reduction: {(1 - final_results['feature_analysis']['selection_summary']['selection_ratio']):.1%}"
        )
