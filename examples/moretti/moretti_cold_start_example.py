...
def generate_demo_data(data_dir):
    ...
    prodotti_info = pd.DataFrame(
        {
            "nome": ["Carrozzina Standard", "Materasso Antidecubito", "Saturimetro Digitale"],
            "categoria": ["Carrozzine", "Materassi Antidecubito", "Elettromedicali"],
...
def cold_start_example():
    ...
    similar_products = cold_start_forecaster.find_similar_products(
        target_product_info=new_product, products_database=products_database, top_n=3
    )
    ...
    if similar_products:
        ...
        for sim in similar_products:
            ...
            print(
                f"      • {sim.source_product}: {product_name} (similarità: {sim.similarity_score:.3f})"
            )
    else:
    ...
    for method in methods:
        ...
        try:
            ...
            results[method] = {"forecast": forecast_series, "metadata": metadata}
    ...
    if results:
        ...
        output_file = (
            output_dir
            / f"cold_start_forecast_{new_product['codice']}_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
        )
        ...
        metadata_file = (
            output_dir
            / f"cold_start_metadata_{new_product['codice']}_{datetime.now().strftime('%Y%m%d_%H%M')}.json"
        )
        import json
