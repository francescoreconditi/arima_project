...
def esempio_semplice_moretti():
    ...
    serie_vendite = pd.Series(
        vendite,
        index=date_range,
        name='Vendite_Carrozzine'
    )
    
    print(f"   Periodo: {date_range[0].strftime('%Y-%m-%d')} - {date_range[-1].strftime('%Y-%m-%d')}")
    ...
    future_dates = pd.date_range(
        start=serie_clean.index[-1] + timedelta(days=1),
        periods=30,
        freq='D'
    )
