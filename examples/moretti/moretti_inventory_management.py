    ...
    def _init_prodotti(self) -> List[ProdottoMedicale]:
        ...
        return [
            ...
            ProdottoMedicale(
                ...
                ProdottoCategoria.ANTIDECUBITO,
                450.0,
                10,
                36,
                15,
            ...
            ProdottoMedicale(
                ...
                ProdottoCategoria.RIABILITAZIONE,
                65.0,
                5,
                None,
                40,
            ...
            ProdottoMedicale(
                ...
                ProdottoCategoria.ELETTROMEDICALI,
                120.0,
                10,
                60,
                25,
    ...
    def _init_fornitori(self) -> Dict[str, List[Fornitore]]:
        ...
        return {
            "CRZ001": [
                ...
                Fornitore("F002", "EuroMedical", 12, 0.92, [(1, 310), (20, 275), (80, 250)]),
            ],
            "MAT001": [
                Fornitore(
                    "F003", "AntiDecubito Pro", 10, 0.98, [(1, 480), (5, 450), (20, 420), (50, 400)]
                ),
                Fornitore("F004", "Medical Comfort", 14, 0.90, [(1, 470), (10, 440), (40, 410)]),
            ],
            # Altri prodotti...
        }

    def genera_serie_vendite(
        self, prodotto: ProdottoMedicale, anni: int = 3, freq: str = "D"
    ) -> pd.Series:
        """
        ...
        - Rumore casuale
        """
        date_range = pd.date_range(end=datetime.now(), periods=365 * anni, freq=freq)

        # Base demand
        base_demand = np.random.poisson(lam=20 + prodotto.criticita * 3, size=len(date_range))
...
class SistemaGestioneScorte:
    ...
    def addestra_modello_prodotto(
        ...
        try:
            ...
            if isinstance(test_forecast, dict):
                test_forecast = (
                    test_forecast["forecast"] if "forecast" in test_forecast else test_forecast
                )
            ...
            if hasattr(test_forecast, "notna") and test_forecast.notna().any():
                metrics = evaluator.calculate_forecast_metrics(
                    actual=serie_clean[-30:].values, predicted=test_forecast
                )
            else:
    ...
    def prevedi_domanda(
        self, prodotto_codice: str, orizzonte_giorni: int = 30, livello_confidenza: float = 0.95
    ) -> pd.DataFrame:
        """
        ...
        try:
            forecast = model.forecast(
                steps=orizzonte_giorni, confidence_intervals=True, alpha=1 - livello_confidenza
            )
    ...
    def ottimizza_fornitore(self, prodotto_codice: str, quantita_necessaria: int) -> Dict:
        """
            ...
            if prezzo_totale_adjusted < migliore_offerta["prezzo_totale"]:
                migliore_offerta = {
                    ...
                    "risparmio_vs_listino": (
                        (fornitore.pricing_tiers[0][1] - prezzo_unitario) * quantita_necessaria
                    ),
                }

        return migliore_offerta

    def genera_piano_approvvigionamento(self, orizzonte_giorni: int = 90) -> pd.DataFrame:
        """
        ...
        for codice, prodotto in self.prodotti.items():
            ...
            scorte_attuali = np.random.randint(prodotto.scorta_minima, prodotto.scorta_minima * 3)
            ...
            for giorno in range(orizzonte_giorni):
                ...
                if (
                    scorte_simulate
                    <= forecast.iloc[min(giorno, len(forecast) - 1)]["punto_riordino"]
                ):
...
def esempio_completo_moretti():
    ...
    sistema = SistemaGestioneScorte(generatore.prodotti, generatore.fornitori)
    ...
    for prodotto in prodotti_critici[:3]:  # Demo con primi 3 prodotti
        ...
        if prodotto.criticita == 5:
            sistema.addestra_modello_prodotto(prodotto.codice, serie_vendite, dati_demografici)
        else:
            sistema.addestra_modello_prodotto(prodotto.codice, serie_vendite)

        # Genera previsioni
        previsioni = sistema.prevedi_domanda(prodotto.codice, orizzonte_giorni=30)
    ...
    if not piano.empty:
        ...
        sintesi = (
            piano.groupby("mese")
            .agg({"costo_totale": "sum", "risparmio": "sum", "prodotto_codice": "count"})
    ...
    try:
        ...
        with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
            ...
            for codice in sistema.previsioni:
                sistema.previsioni[codice].to_excel(writer, sheet_name=f"Previsioni_{codice}")
    except ImportError:
