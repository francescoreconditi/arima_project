    ...
    if not FORECASTING_AVAILABLE:
        ...
        **Advanced Exogenous Analysis** consente di utilizzare variabili esterne (exogenous)
        ...
        subtab1, subtab2, subtab3, subtab4 = st.tabs(
            ["📊 Relationships", "🔧 Preprocessing", "🔍 Diagnostics", "🎯 Feature Selection"]
        )
                    ...
                    for feature in exog_demo.columns:
                        try:
                            p_value = diagnostics.granger_causality_test(
                                target_series, exog_demo[feature], maxlag=7
                            )
            ...
            if prodotto_sel != "Tutti":
                ...
                with col2:
                    max_features = st.slider(
                        ...
                        1,
                        len(exog_demo.columns),
                        min(3, len(exog_demo.columns)),
                        help="Numero massimo di features da selezionare",
                    )

                    test_size = (
                        st.slider(
                            "Test Size (%):", 10, 50, 20, help="Percentuale dati per test set"
                        )
                        / 100
                    )
                ...
                if st.button("🚀 Run Advanced SARIMAX Analysis", type="primary"):
                    with st.spinner("Running SARIMAX Auto-Selection..."):
                        try:
                            ...
                            auto_selector.fit_with_exog(y_train, X_train, validate_input=False)
                            ...
                            try:
                                ...
                                mape = (
                                    np.mean(np.abs((y_test - forecast) / np.maximum(y_test, 1)))
                                    * 100
                                )
                            ...
                            st.error(
                                "Ensure you have sufficient historical data and valid exogenous variables."
                            )
            else:
