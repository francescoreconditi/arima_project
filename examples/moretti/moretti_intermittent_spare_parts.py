...
def genera_domanda_intermittente(n_periods=365, 
                                 tipo='spare_part',
                                 seed=42):
    """
...
def analizza_ricambio(codice, nome, domanda_storica, 
                      costo_unitario, lead_time_giorni,
    ...
    print(f"Giorni con domanda: {sum(train_data > 0)} ({100*sum(train_data > 0)/len(train_data):.1f}%)")
...
def esempio_portfolio_ricambi():
    ...
    for ricambio in ricambi:
        # Genera dati storici
        domanda = genera_domanda_intermittente(
            n_periods=365,
            tipo=ricambio['pattern']
        )
        ...
        if risultati:
            best_method = min(risultati.items(), 
                            key=lambda x: x[1]['metrics'].mase if x[1] else float('inf'))
