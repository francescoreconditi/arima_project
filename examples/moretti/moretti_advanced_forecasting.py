...
class FattoriEsterniManager:
    ...
    def carica_dati_epidemiologici_simulati(self) -> pd.DataFrame:
        ...
        if covid_start < n_periods:
            covid[covid_start:covid_start+12] = stats.gamma.pdf(
                np.arange(12), a=3, scale=2
            ) * 5000
        # Onde successive
        for wave_start in [covid_start+30, covid_start+50, covid_start+70]:
            if wave_start + 8 < n_periods:
                covid[wave_start:wave_start+8] = stats.gamma.pdf(
                    np.arange(8), a=2, scale=2
                ) * 2000
...
class ModelloVARMultiProdotto:
    ...
    def prepara_dataset_var(
        self,
        vendite_dict: Dict[str, pd.Series],
            ...
            for fattore in dataset.columns:
                ...
                try:
                    ...
                    gc_result = grangercausalitytests(
                        test_data, 
                        maxlag=min(max_lag, len(test_data)//5),
                        verbose=False
                    )
                    
                    # Estrai p-value minimo
                    p_values = [gc_result[lag][0]['ssr_ftest'][1] for lag in range(1, len(gc_result)+1)]
                    min_p = min(p_values)
            ...
            for j, response_var in enumerate(var_names):
                if response_var in self.prodotti:
                    ...
                    risultati.append({
                        ...
                        'durata_effetto': np.where(np.abs(response) < 0.01)[0][0] if any(np.abs(response) < 0.01) else periodi
                    })
    ...
    def verifica_obsolescenza(
        ...
        if giorni_rimanenti <= 60:  # Alert 2 mesi prima
            return {
                ...
                'azione_suggerita': 'Promozione smaltimento' if giorni_rimanenti <= 30 else 'Ridurre riordini'
            }
        return None
    ...
    def verifica_anomalie_domanda(
        ...
        if deviazione > soglia_deviazione:
            return {
                ...
                'azione_suggerita': 'Rivedere previsioni' if domanda_attuale > domanda_prevista else 'Verificare disponibilità'
            }
        return None
...
def esempio_avanzato_moretti():
    ...
    vendite = {
        'CRZ001': pd.Series(
            np.random.poisson(15, len(date_range)) * (1 + 0.3*np.sin(np.arange(len(date_range))*2*np.pi/365)),
            index=date_range
        ),
        'MAT001': pd.Series(
            np.random.poisson(12, len(date_range)) * (1 + 0.2*np.sin(np.arange(len(date_range))*2*np.pi/365 + np.pi/4)),
            index=date_range
        ),
        'RIA001': pd.Series(
            np.random.poisson(20, len(date_range)) * (1 + 0.15*np.sin(np.arange(len(date_range))*2*np.pi/365 - np.pi/3)),
            index=date_range
        )
    }
    ...
    for prodotto, fattori in causalita.items():
        ...
        if significativi:
            for fattore, info in sorted(significativi.items(), key=lambda x: x[1]['p_value'])[:3]:
                print(f"      • {fattore}: p-value={info['p_value']:.4f}, lag={info['optimal_lag']}")
    ...
    if irf_results is not None and not irf_results.empty:
        ...
        for _, row in top_interactions.iterrows():
            if row['shock'] != row['risposta']:
                print(f"      • {row['shock']} → {row['risposta']}: impatto max={row['impatto_max']:.3f} al giorno {row['periodo_max']}")
