...
def calcola_kpi_business(vendite_df, previsioni_df):
    ...
    kpis = {
        ...
        'domanda_media_giorno': vendite_df.groupby('prodotto_codice')['quantita_venduta'].mean().mean(),
...
def crea_dashboard_html(vendite_df, previsioni_df, riordini_df, kpis):
    ...
    for i, codice in enumerate(previsioni_df['prodotto_codice'].unique()):
        ...
        fig_forecast.add_trace(go.Scatter(
            ...
            y=data_prev['confidence_upper'].tolist() + data_prev['confidence_lower'][::-1].tolist(),
            fill='tonexty',
    ...
    fig_investimenti = go.Figure(data=[
        go.Bar(
