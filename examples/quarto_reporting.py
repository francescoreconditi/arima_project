...
from arima_forecaster import (
    ...
    TimeSeriesPreprocessor, DataLoader,
    ForecastPlotter, ModelEvaluator
)
...
try:
    ...
    HAS_REPORTING = False
    print("[WARN] Moduli di reporting non disponibili. Installare con: pip install 'arima-forecaster[reports]'")
...
def create_visualizations(arima_model: ARIMAForecaster, sarima_model: SARIMAForecaster, 
                         data: pd.Series) -> dict:
    ...
    try:
        ...
        forecast_data = arima_forecast['forecast'] if isinstance(arima_forecast, dict) else arima_forecast
        conf_int = arima_forecast.get('confidence_intervals') if isinstance(arima_forecast, dict) else None
        ...
        forecast_data = sarima_forecast['forecast'] if isinstance(sarima_forecast, dict) else sarima_forecast
        conf_int = sarima_forecast.get('confidence_intervals') if isinstance(sarima_forecast, dict) else None
...
def generate_individual_reports(arima_model: ARIMAForecaster, sarima_model: SARIMAForecaster,
                               plot_files: dict):
...
def generate_comparison_report(arima_model: ARIMAForecaster, sarima_model: SARIMAForecaster,
                              data: pd.Series, plot_files: dict):
