...
def generate_web_traffic_data():
    ...
    monthly_seasonal = np.array(
        [
            1.3
            if ts.month in [1, 9, 10]
            # New Year, Back to school, Q4
            else 1.1
            if ts.month in [3, 4, 11]
            # Q1 end, Q2 start, Black Friday
            else 0.8
            if ts.month in [7, 8, 12]
            # Summer, holidays
            else 1.0  # Altri mesi
            for ts in timestamps
        ]
    ...
    for update_date in algo_update_dates:
        ...
        if 0 <= update_start < n_points:
            ...
            recovery_curve = np.linspace(
                initial_drop, np.random.uniform(0.9, 1.1), update_end - update_start
            )
...
def main():
    ...
    print(
        f"📅 Periodo: {traffic_data.index[0].strftime('%Y-%m-%d')} - {traffic_data.index[-1].strftime('%Y-%m-%d')}"
    )
    ...
    print(f"\n🔄 Split dataset:")
    print(
        f"  📚 Training: {len(train_data):,} ore ({train_data.index[0].strftime('%Y-%m-%d')} - {train_data.index[-1].strftime('%Y-%m-%d')})"
    )
    print(
        f"  🧪 Test: {len(test_data):,} ore ({test_data.index[0].strftime('%Y-%m-%d')} - {test_data.index[-1].strftime('%Y-%m-%d')})"
    )
    ...
    logger.info(
        f"🔮 Generazione forecast per {forecast_steps} ore ({forecast_steps / 24:.1f} giorni)..."
    )
    ...
    forecast_peak_hours = (
        forecast_result["forecast"].groupby(forecast_result["forecast"].index.hour).mean()
    )
    ...
    ax2.plot(test_data.index, test_data, label="Test Actual", color="green", alpha=0.8, linewidth=1)
    ax2.plot(
        test_data.index,
    ...
    hourly_pattern_forecast = (
        forecast_result["forecast"].groupby(forecast_result["forecast"].index.hour).mean()
    )

    ax3.plot(
        hourly_pattern_train.index,
        hourly_pattern_train.values,
        "o-",
        label="Training Avg",
        color="blue",
        linewidth=2,
    ...
    ax3.plot(
        hourly_pattern_test.index,
        hourly_pattern_test.values,
        "s-",
        label="Test Actual",
        color="green",
        linewidth=2,
    ...
    ax3.plot(
        hourly_pattern_forecast.index,
        hourly_pattern_forecast.values,
        "^-",
        label="Forecast",
        color="red",
        linewidth=2,
    ...
    ax4.bar(
        ...
        weekly_pattern_train.values,
        width,
        label="Training",
        alpha=0.7,
        color="blue",
    )
    ax4.bar(x_pos, weekly_pattern_test.values, width, label="Test Actual", alpha=0.7, color="green")
    ax4.bar(
        [x + width for x in x_pos],
        weekly_pattern_forecast.values,
        width,
        label="Forecast",
        alpha=0.7,
        color="red",
    ...
    ax5.scatter(
        test_data.index[special_events_mask],
    ...
    ax6.plot(
        actual_smooth.index, actual_smooth, label="Actual (24h avg)", color="green", linewidth=2
    )
    ax6.plot(
        forecast_smooth.index,
        forecast_smooth,
        label="Forecast (24h avg)",
        color="red",
        linestyle="--",
        linewidth=2,
    )
    ...
    ax7_twin.plot(mobile_trend.index, mobile_trend, color="red", linewidth=2, label="Mobile Trend")
    ...
    future_dates = pd.date_range(
        start=traffic_data.index[-1] + pd.Timedelta(hours=1), periods=future_steps, freq="h"
    )
    ...
    ax9.plot(
        test_data.index, residuals, "o-", color="orange", alpha=0.7, markersize=1.5, linewidth=0.8
    )
    ...
    print(
        f"  🏆 Miglior giorno previsto: {best_day.strftime('%A %Y-%m-%d')} ({future_daily.max():,.0f} visitatori)"
    )
    print(
        f"  📉 Giorno più basso: {worst_day.strftime('%A %Y-%m-%d')} ({future_daily.min():,.0f} visitatori)"
    )
    ...
    if pd.isna(peak_hour) or pd.isna(peak_traffic):
        ...
        print(
            f"  ⏰ Ora di picco media: {int(peak_hour):02d}:00 ({peak_traffic:,.0f} visitatori/ora)"
        )
    ...
    # Salva modello
    model_path = get_models_path("web_traffic_arima_model.joblib")
