...
def main():
    ...
    if not comparison.empty:
        print(
            comparison[["Model", "Accuracy_Score", "Confidence", "Status"]].to_string(index=False)
        )
    ...
    print(
        f"# Confidence: {explanation.confidence_score:.1%} | Model: {explanation.recommended_model}"
    )
...
def business_example():
    ...
    for product, pattern in products.items():
        ...
        print(
            f"   [OK] {explanation.recommended_model} ({explanation.confidence_score:.1%} conf, {elapsed:.1f}s)"
        )
    ...
    print(
        f"   All models selected with >70% confidence: {'[OK]' if all(r['confidence'] > 0.7 for r in results.values()) else '[WARN]'}"
    )
