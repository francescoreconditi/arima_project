...
def generate_manufacturing_data():
    ...
    weekend_reduction = np.array([0.3 if ts.weekday() >= 5 else 1.0 for ts in timestamps])
    ...
    for i, failure in enumerate(failure_events):
        if failure:
            ...
            failure_duration = np.random.choice(
                [1, 2, 3, 4, 6, 8, 12], p=[0.3, 0.25, 0.2, 0.15, 0.05, 0.03, 0.02]
            )
...
def calculate_manufacturing_kpis(production, metadata):
    ...
    kpis = {
        ...
        "mtbf": len(production)
        / max(1, (1 - metadata["unplanned_downtime"]).sum()),  # Mean Time Between Failures
        "mttr": unplanned_downtime_hours
        / max(1, len(metadata[metadata["unplanned_downtime"] < 1])),  # Mean Time To Repair
...
def main():
    ...
    print(
        f"📅 Periodo: {production_data.index[0].strftime('%Y-%m-%d %H:%M')} - {production_data.index[-1].strftime('%Y-%m-%d %H:%M')}"
    )
    ...
    print(
        f"🔍 Ore con produzione zero: {zero_production} ({zero_production / len(train_data) * 100:.1f}%)"
    )
    ...
    rolling_availability = (
        (metadata["planned_downtime"] * metadata["unplanned_downtime"]).rolling(window).mean()
    )
    ...
    ax3.bar(hourly_avg.index, hourly_avg.values, yerr=hourly_std.values, alpha=0.7, capsize=3)
    ...
    if downtime_events:
        ...
        ax5.axvline(
            x=np.mean(downtime_events),
            color="blue",
            linestyle="--",
            label=f"Mean: {np.mean(downtime_events):.1f}h",
        )
    ...
    ax6.plot(
        efficiency_trend.index,
        efficiency_trend,
        color="purple",
        linewidth=2,
        label="Line Efficiency (24h)",
    )
    ...
    ax7.plot(recent_data.index, recent_data, label="Recent Data", color="blue", alpha=0.7)

    # Future forecast
    future_dates = pd.date_range(
        start=production_data.index[-1] + pd.Timedelta(hours=1), periods=future_steps, freq="h"
    )
    ...
    ax8.plot(test_data.index, residuals, "o-", color="orange", alpha=0.7, markersize=2)
    ...
    ax8.scatter(
        test_data.index[unplanned_down_mask],
    ...
    ax8.scatter(
        test_data.index[material_shortage_mask],
    ...
    ax8.scatter(
        test_data.index[quality_issue_mask],
    ...
    # Salva plot
    plot_path = get_plots_path("manufacturing_forecast.png")
    ...
    print(
        f"  📈 vs settimana precedente: {((future_production_total - current_week_total) / current_week_total * 100):+.1f}%"
    )
    ...
    print(
        f"  📊 Migliore giorno previsto: Giorno {best_day + 1} ({daily_forecasts[best_day]:,.0f} unità)"
    )
    print(
        f"  📊 Giorno più critico: Giorno {worst_day + 1} ({daily_forecasts[worst_day]:,.0f} unità)"
    )

    # Maintenance recommendations
    current_efficiency = metadata["line_efficiency"].iloc[-168:].mean()  # Last week
    efficiency_trend = (
        metadata["line_efficiency"].iloc[-168:].mean()
        - metadata["line_efficiency"].iloc[-336:-168].mean()
    )
