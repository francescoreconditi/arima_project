...
def crea_lotti_demo(prodotto_codice: str):
    ...
    for lotto in lotti:
        print(f"- {lotto.lotto_id}: {lotto.quantita} unità, "
...
def configura_domanda_e_prezzi():
    ...
    for canale in domanda_canali:
        print(f"- {canale.replace('_', ' ').title()}: "
...
def esegui_ottimizzazione_msl():
    ...
    print(f"   Quantità allocata: {totale_allocato} / {sum(l.quantita for l in lotti)} unità "
    ...
    for giorni in test_giorni:
        ...
        print(f"   Prodotto con {giorni} giorni residui → {len(canali_compatibili)} canali: "
...
def analisi_what_if():
    ...
    valore_aggressivo = sum(
        sum(a.valore_allocato for a in allocazioni)
        for allocazioni in risultati_aggressivo.values()
    )
