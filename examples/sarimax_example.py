...
def generate_sample_data_with_exog(n_periods: int = 200) -> tuple[pd.Series, pd.DataFrame]:
    ...
    temperature = (
        20
        + 10 * np.sin(2 * np.pi * np.arange(n_periods) / 365.25)
    ...
    # Stagionalità
    seasonal = 50 * np.sin(2 * np.pi * np.arange(n_periods) / 365.25) + 20 * np.sin(
        2 * np.pi * np.arange(n_periods) / 7
    )
...
def demonstrate_sarimax_basic():
    ...
    try:
        ...
        if not exog_importance.empty:
            ...
            for _, row in exog_importance.iterrows():
                ...
                print(
                    f"   {row['variable']}: coeff={row['coefficient']:.4f}, p-value={row['pvalue']:.4f} {significance}"
                )
    ...
    exog_future_processed, _ = preprocessor.preprocess_exog_data(
        exog=exog_future, handle_missing=True, scale_data=True, scaling_method="standardize"
    )
    ...
    try:
        forecast_result = model.forecast(
            steps=forecast_steps, exog_future=exog_future_processed, confidence_intervals=True
        )
        ...
        if isinstance(forecast_result, tuple):
            ...
            conf_int = None
            print(
                f"Previsioni generate: {len(forecast_values)} punti (senza intervalli confidenza)"
            )
        ...
        if len(forecast_values) <= len(series_test):
            ...
            try:
                metrics = evaluator.calculate_forecast_metrics(
                    actual=actual_values.values, predicted=forecast_values.values
                )
...
def demonstrate_sarimax_model_selection():
    ...
    series_processed, exog_processed, _ = preprocessor.preprocess_pipeline_with_exog(
        series=series, exog=exog, make_stationary_flag=True, scale_exog=True
    )
    ...
    # Esegui ricerca
    selector.search(series=series_processed, exog=exog_processed, verbose=True)
    ...
    if best_model:
        ...
        try:
            ...
            if exog_analysis is not None:
                ...
                significant_vars = exog_analysis[exog_analysis["significant"] == True][
                    "variable"
                ].unique()
...
def demonstrate_sarimax_visualization():
    ...
    preprocessor = TimeSeriesPreprocessor()
    series_processed, exog_processed, _ = preprocessor.preprocess_pipeline_with_exog(
        series=series_train, exog=exog_train, scale_exog=True
    )

    # Addestra modello
    print("\n[TRAINING] Addestramento modello per visualizzazioni...")
    model = SARIMAXForecaster(
        order=(1, 1, 1), seasonal_order=(1, 0, 1, 7), exog_names=list(exog.columns)
    )
    model.fit(series_processed, exog=exog_processed)
