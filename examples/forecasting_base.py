...
def generate_sample_data():
    ...
    # Crea DataFrame
    df = pd.DataFrame({"date": dates, "sales": values}).set_index("date")
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Serie originale è stazionaria: {stationarity_test['is_stationary']} (p-value: {stationarity_test['p_value']:.4f})"
        )
        ...
        logger.info(
            f"Preprocessing completato: {len(series)} -> {len(processed_series)} osservazioni"
        )
        ...
        forecast, conf_int = model.forecast(
            steps=forecast_steps, confidence_intervals=True, return_conf_int=True
        )
        ...
        future_forecast, future_conf_int = full_model.forecast(
            steps=12, confidence_intervals=True, return_conf_int=True
        )
        ...
        for i, (f, lower, upper) in enumerate(
            zip(future_forecast, future_conf_int.iloc[:, 0], future_conf_int.iloc[:, 1])
        ):
