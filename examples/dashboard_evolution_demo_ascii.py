...
def main():
    ...
    # Generate report
    excel_data = exporter.generate_procurement_report(forecast_data, inventory_params, product_info)
    ...
    # Marketing scenario
    params = ScenarioParameters(marketing_boost=100.0, price_change=-15.0, seasonality_factor=1.3)
    ...
    # Save Excel file
    output_file = (
        Path(__file__).parent / f"demo_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    )
