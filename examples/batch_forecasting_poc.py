...
def generate_realistic_portfolio():
    ...
    portfolio['Prodotto_Seasonal'] = pd.Series(seasonal_base + weekly_pattern + monthly_pattern + seasonal_noise)
    ...
    for name, series in portfolio.items():
        print(f"   {name}: {len(series)} obs, media={series.mean():.1f}, zeri={((series == 0).sum() / len(series) * 100):.1f}%")
...
def run_batch_processing_poc():
    ...
    def progress_callback(progress):
        ...
        if completion > 0 and completion % 0.2 < 0.01:  # Ogni 20%
            print(f"   📊 Progress: {completion:.1%} ({progress.completed_tasks}/{progress.total_tasks}) - ETA: {progress.estimated_completion:.1f}s")
    
    # 4. Avvia batch processing
    print(f"\n🔥 Avvio Batch Processing su {len(portfolio)} serie...")
    print(f"   Configurazione: {processor.max_workers} workers, {processor.max_models_to_try} modelli/serie")
    ...
    for name, result in results.items():
        if result.status == 'success':
            summary_data.append({
                ...
                'forecast_mean': np.mean(result.forecast) if result.forecast is not None else None,
                'forecast_std': np.std(result.forecast) if result.forecast is not None else None
            })
...
def performance_summary(results, processing_time, progress_log):
    ...
    print(f"  Efficiency:           {(avg_training_time * total_series) / processing_time:.1f}x speedup")
    ...
    print(f"  Alta confidence >80%: {high_confidence}/{len(successful_results)} ({high_confidence/len(successful_results):.1%})")
