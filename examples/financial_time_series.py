...
def generate_financial_data():
    ...
    for i in range(1, n_days):
        ...
        volatility[i] = np.sqrt(0.000005 + 0.05 * (volatility[i-1]**2) + 0.90 * volatility[i-1]**2)
...
def main():
    ...
    print(f"📅 Periodo: {price_data.index[0].strftime('%Y-%m-%d')} - {price_data.index[-1].strftime('%Y-%m-%d')}")
    ...
    plt.plot(test_data.index, forecast_result['forecast'], 
             's-', label='Forecast', color='red', markersize=2)
    ...
    plt.plot(actual_returns.index, actual_returns, 'o-', 
             label='Actual Returns', color='green', alpha=0.7, markersize=2)
    ...
    plt.plot(future_dates, future_prices, 
             's-', label='Future Forecast', color='purple', linewidth=2, markersize=4)
    ...
    # Salva plot
    plt.savefig(get_plots_path('financial_forecast.png'), dpi=300, bbox_inches='tight')
    ...
    print(f"  📊 Confidence interval 30gg: ${future_lower.iloc[-1]:.2f} - ${future_upper.iloc[-1]:.2f}")
