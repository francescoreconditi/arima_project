...
def demo_excel_export():
    ...
    # Save report
    output_file = Path(__file__).parent / f"procurement_report_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
    ...
    quick_file = Path(__file__).parent / f"quick_summary_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
...
def demo_integration():
    ...
    # 4. Export results
    final_file = Path(__file__).parent / f"integrated_workflow_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
