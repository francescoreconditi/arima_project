...
def demo_perishable_management():
    ...
    for lotto in lotti_analizzati:
        print(
            f"{lotto.lotto_id}: {lotto.quantita} unità - {lotto.giorni_residui} giorni rimanenti "
    ...
    markdown_info = perishable_mgr.calcola_markdown_ottimale(
        lotto_critico, domanda_giornaliera, elasticita_prezzo=2.0
    )
    ...
    strategia = perishable_mgr.strategia_riordino_perishable(
        lotti_analizzati, forecast, shelf_life_nuovo_lotto=365, costo_obsolescenza_unitario=8.0
    )
    ...
    print(
        f"Stock a rischio: {strategia['stock_a_rischio']} unità ({strategia['percentuale_a_rischio']:.1f}%)"
    )
...
def demo_multi_echelon():
    ...
    for nodo_id in ["CENTRAL", "REG_NORD", "LOC_MI"]:
        ss_info = optimizer.calcola_safety_stock_echelon(
            nodo_id, service_level_target=0.95, variabilita_domanda=25, variabilita_lead_time=0.15
        )
    ...
    richieste_regionali = {"REG_NORD": 3500, "REG_CENTRO": 2800, "REG_SUD": 2200}

    # Priorità (Nord ha priorità più alta)
    priorita = {"REG_NORD": 1.5, "REG_CENTRO": 1.0, "REG_SUD": 1.2}

    stock_centrale_disponibile = 8000

    allocation = optimizer.ottimizza_allocation(
        stock_centrale_disponibile, richieste_regionali, priorita
    )
    ...
    for nodo_id, dettaglio in allocation["dettaglio_nodi"].items():
        print(
            f"  {nodo_id}: {dettaglio['allocato']}/{dettaglio['richiesta']} "
    ...
    if shortage_info["fattibile"]:
        ...
        for t in shortage_info["transshipments"]:
            print(
                f"  {t['da_nodo']} -> {t['a_nodo']}: {t['quantita']} unita (E{t['costo_totale']:.0f})"
            )
    else:
...
def demo_capacity_constraints():
    ...
    inventario_attuale = {"PROD_A": 2000, "PROD_B": 800, "PROD_C": 5000}
    ...
    for vincolo_id, info in utilizzi.items():
        ...
        print(
            f"  Utilizzo: {info['utilizzo_assoluto']:.1f}/{info['capacita_massima']:.0f} {vincolo.unita_misura} "
    ...
    ottimizzazione = optimizer.ottimizza_con_vincoli(richieste_riordino, priorita_prodotti)
    ...
    for prod_id, qty_richiesta in richieste_riordino.items():
        ...
        print(f"  {prod_id}: {qty_approvata}/{qty_richiesta} ({qty_approvata / qty_richiesta:.0%})")
    ...
    if richieste_non_soddisfatte:
        espansioni = optimizer.suggerisci_espansione_capacita(
            richieste_non_soddisfatte, orizzonte_mesi=12
        )
        ...
        for vincolo_id, info in espansioni["espansioni_consigliate"].items():
            ...
            print(
                f"    Capacità extra: {info['capacita_extra_necessaria']:.1f} {vincoli_magazzino[vincolo_id].unita_misura}"
            )
...
def demo_kitting_bundle():
    ...
    if kit_info["limitazioni_dettaglio"]:
        ...
        for comp_id, info in kit_info["limitazioni_dettaglio"].items():
            print(
                f"  {comp_id}: {info['stock_disponibile']} stock → {info['kit_possibili']} kit possibili"
            )
    ...
    analisi_strategica = kitting_optimizer.ottimizza_kit_vs_componenti(
        "KIT_MEDICO_BASE", forecast_kit, forecast_componenti_separati
    )
