...
def analizza_prodotto(prodotto, vendite, costi):
    ...
    if categoria_movimento in [
        CategoriaMovimentazione.SLOW_MOVING,
        CategoriaMovimentazione.VERY_SLOW,
    ]:
        ottimizzazione = optimizer.optimize_slow_moving(
            vendite[-30:], prodotto["prezzo_unitario"], prodotto["lead_time"]
        )
    ...
    # Costo ottimizzato
    costo_ottimizzato = (
        ottimizzazione["safety_stock"] * prodotto["prezzo_unitario"] * costi.tasso_capitale
    )
...
def main():
    ...
    for _, row in risultati_df.iterrows():
        print(
            f"{row['product_id']}: {row['nome'][:30]:<30} | "
