...
def generate_user_engagement_data():
    ...
    monthly_dau_pattern = np.array([
        1.2 if date.month in [1, 9] else  # New Year, Back to school
        0.8 if date.month in [7, 8, 12] else  # Summer holidays, Christmas
        1.0  # Normal months
        for date in dates
    ])
    ...
    for release_day in release_dates:
        if release_day < n_days:
            ...
            boost_curve = boost_curve * np.exp(-np.linspace(0, 2, end_period - release_day))  # Decay
    ...
    # Calculate DAU
    dau = (total_users * base_dau_rate * weekly_dau_pattern * 
           monthly_dau_pattern * feature_releases * marketing_campaigns * 
           churn_events * daily_noise)
    ...
    weekend_session_boost = np.array([
        1.0 if date.weekday() < 5 else 1.3 for date in dates
    ])
    ...
    session_duration = (base_session_duration * user_dilution_effect * 
                       feature_richness * weekend_session_boost * 
                       seasonal_session * session_noise)
    ...
    retention_rate = (base_retention * retention_boost * retention_impact * 
                     onboarding_improvements * retention_noise)
    ...
    ab_testing_effects = 1 + 0.1 * np.sin(2 * np.pi * np.arange(n_days) / 30) * \
                        np.random.choice([0, 1], n_days, p=[0.7, 0.3])  # Only during testing
    
    adoption_noise = np.random.normal(1.0, 0.12, n_days)
    
    feature_adoption = (base_adoption * adoption_growth * pricing_changes * 
                       ab_testing_effects * adoption_noise)
    ...
    engagement_multiplier = (session_duration / base_session_duration) * \
                           (retention_rate / base_retention) * 0.5 + 0.5
    
    # Seasonal spending patterns
    seasonal_spending = np.array([
        1.5 if date.month == 12 else  # December holiday spending
        1.2 if date.month in [6, 7] else  # Summer spending
        0.9 if date.month in [1, 2] else  # Post-holiday drop
        1.0
        for date in dates
    ])
    ...
    revenue_per_user = (base_rpu * engagement_multiplier * seasonal_spending * 
                       economic_trend * rpu_noise)
...
def calculate_engagement_kpis(engagement_data, metadata):
    ...
    avg_mau = engagement_data['daily_active_users'].rolling(30).mean().iloc[-1] * 30  # Approssimazione
    dau_mau_ratio = avg_dau / (avg_mau / 30) if avg_mau > 0 else 0
    
    # Session metrics
    avg_session_duration = engagement_data['session_duration_min'].mean()
    session_duration_trend = (engagement_data['session_duration_min'].iloc[-30:].mean() - 
                             engagement_data['session_duration_min'].iloc[:30].mean())
    
    # Retention analysis
    avg_retention = engagement_data['retention_rate_7d'].mean()
    retention_trend = (engagement_data['retention_rate_7d'].iloc[-30:].mean() - 
    ...
    # Growth metrics
    user_growth = ((engagement_data['total_user_base'].iloc[-1] - 
                   engagement_data['total_user_base'].iloc[0]) / 
                   engagement_data['total_user_base'].iloc[0])
    
    # Volatility metrics
    dau_volatility = engagement_data['daily_active_users'].std() / avg_dau
    revenue_volatility = engagement_data['total_revenue'].std() / engagement_data['total_revenue'].mean()
...
def main():
    ...
    print(f"📅 Periodo: {engagement_data.index[0].strftime('%Y-%m-%d')} - {engagement_data.index[-1].strftime('%Y-%m-%d')}")
    ...
    print(f"\n🔄 Split dataset (DAU):")
    print(f"  📚 Training: {len(train_data)} giorni ({train_data.index[0].strftime('%Y-%m-%d')} - {train_data.index[-1].strftime('%Y-%m-%d')})")
    print(f"  🧪 Test: {len(test_data)} giorni ({test_data.index[0].strftime('%Y-%m-%d')} - {test_data.index[-1].strftime('%Y-%m-%d')})")
    ...
    weekend_mape = np.mean(np.abs((test_data[weekend_mask] - forecast_result['forecast'][weekend_mask]) / 
                                 test_data[weekend_mask])) * 100
    weekday_mape = np.mean(np.abs((test_data[~weekend_mask] - forecast_result['forecast'][~weekend_mask]) / 
                                 test_data[~weekend_mask])) * 100
    ...
    dau_session_corr = np.corrcoef(engagement_data['daily_active_users'], 
                                   engagement_data['session_duration_min'])[0, 1]
    ...
    retention_dau_corr = np.corrcoef(engagement_data['daily_active_users'], 
                                     engagement_data['retention_rate_7d'])[0, 1]
    ...
    ax1.plot(test_data.index, forecast_result['forecast'], 
    ...
    for i, metric in enumerate(metrics_to_plot):
        ax2.plot(normalized_data.index, normalized_data[metric], 
                label=metric.replace('_', ' ').title(), linewidth=2, alpha=0.8)
    ...
    weekly_forecast = forecast_result['forecast'].groupby(forecast_result['forecast'].index.dayofweek).mean()
    
    days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
    x = range(7)
    width = 0.25
    
    ax3.bar([i - width for i in x], weekly_dau.values, width, 
            label='Training', alpha=0.7, color='blue')
    ax3.bar(x, weekly_test.values, width, 
            label='Test Actual', alpha=0.7, color='green')
    ax3.bar([i + width for i in x], weekly_forecast.values, width, 
    ...
    bars = ax4.bar(monthly_revenue.index, monthly_revenue.values / 1000, 
                   alpha=0.7, color='green', label='Monthly Revenue ($k)')
    ax4.set_ylabel('Revenue ($k)', color='green')
    ax4.tick_params(axis='y', labelcolor='green')
    
    line = ax4_twin.plot(monthly_dau.index, monthly_dau.values / 1000, 
                         'ro-', color='red', label='Avg DAU (k)')
    ...
    ax5.plot(engagement_data.index, engagement_data['feature_adoption_rate'] * 100,
             color='purple', linewidth=2, label='Feature Adoption %')
    
    # Evidenzia periodi marketing campaigns
    campaign_mask = metadata['marketing_campaigns'] > 1.2
    if campaign_mask.any():
        ax5.fill_between(engagement_data.index, 0, 50,
                         where=campaign_mask, alpha=0.3, color='orange',
    ...
    ax6.plot(engagement_data.index, retention_smooth * 100,
             color='orange', linewidth=2, label='7-Day Retention % (7d avg)')
    
    # Mark significant retention drops
    retention_drops = engagement_data['retention_rate_7d'].diff() < -0.05
    if retention_drops.any():
        ax6.scatter(engagement_data.index[retention_drops], 
                    (engagement_data['retention_rate_7d'][retention_drops] * 100),
                    color='red', s=50, alpha=0.8, label='Retention Drops', zorder=5)
    ...
    ax7.hist(weekday_sessions, bins=30, alpha=0.7, color='blue', 
             label=f'Weekdays (μ={weekday_sessions.mean():.1f}m)', density=True)
    ax7.hist(weekend_sessions, bins=30, alpha=0.7, color='green', 
             label=f'Weekends (μ={weekend_sessions.mean():.1f}m)', density=True)
    ...
    ax8.plot(recent_dau.index, recent_dau, 
             label='Recent DAU', color='blue', linewidth=2)
    
    # Future dates
    future_dates = pd.date_range(
        start=dau_series.index[-1] + pd.Timedelta(days=1),
        periods=future_steps, 
        freq='D'
    )
    
    ax8.plot(future_dates, future_dau, 
             'o-', label='30-Day Forecast', color='purple', 
    ...
    ax9.scatter(test_data, forecast_result['forecast'], 
    ...
    bars1 = ax10.bar([i - width/2 for i in x], current_values, width,
                     label='Current (30d avg)', alpha=0.7, color='blue')
    bars2 = ax10.bar([i + width/2 for i in x], future_values, width,
                     label='Forecast (30d)', alpha=0.7, color='red')
    ...
    # Revenue projections
    future_revenue_estimate = future_monthly_dau * engagement_data['revenue_per_user'].iloc[-30:].mean() * 30
    ...
    if current_adoption < 0.20:
        print(f"  🎯 Feature adoption bassa ({current_adoption:.1%}) - ottimizzare onboarding premium")
    ...
    if recent_volatility > 0.15:
        print(f"  🚨 VOLATILITY ALERT: DAU volatility {recent_volatility:.1%} - investigate churn factors")
    else:
